<?
	...
	if ($searchFor=='listing') {
		...
		if ($searchFor) {
			$pageObj = new pageBrowsing("Promotion", $screen, $search_limit, "name", "name", $letter, $where, "Promotion.*", "Promotion");
        ...
        if ($searchFor) {
            $pageObj = new pageBrowsing("Post", $screen, $search_limit, "title DESC", "title", $letter, $where);
	...
	if (${$feature_plural}) { ?>
		...
		<? include(INCLUDES_DIR."/tables/table_".string_strtolower($searchFor).".php"); ?>
