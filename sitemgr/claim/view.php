<?
	...
	$url_redirect = "".DEFAULT_URL."/".SITEMGR_ALIAS."/claim";
	...
	# CODE
	# ----------------------------------------------------------------------------------------------------
	$errorPage = DEFAULT_URL."/".SITEMGR_ALIAS."/claim/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "");
	if ($id) {
		$claim = new Claim($id);
	...
	<div id="content-content">
		<div class="default-margin">
				...
				<tr>
					...
					<td>
						...
						if ($claim->getNumber("account_id")) {
							echo "<a href=\"".$url_base."/account/view.php?id=".$claim->getNumber("account_id")."\">";
						}
				...
				<tr>
					...
					<td>
						...
						if ($claim->getNumber("listing_id")) {
							echo "<a href=\"".$url_base."/".LISTING_FEATURE_FOLDER."/view.php?id=".$claim->getNumber("listing_id")."\">";
						}
			...
			if ($claim->getNumber("listing_id")) {
				...
				?><a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=LISTING_FEATURE_FOLDER;?>/preview.php?id=<?=$listing->getNumber("id")?>" class="standardLINK iframe fancy_window_preview"><?=system_showText(LANG_SITEMGR_CLICKHERETOPREVIEW)?> <?=system_showText(LANG_SITEMGR_LISTING)?></a><?
			}
			?>
			...
			<table class="view-claim-table">
				...
				<tr>
					<td class="claim-listing-label"><?=ucfirst(system_showText(LANG_SITEMGR_TEMPLATE))?>:</td>
