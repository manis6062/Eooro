			...
			if ($_POST['email_notification'] == 1 && $_POST['status'] == "A") {
				if ($articleObj->getNumber("account_id") > 0) {
					...
					if ($emailNotificationObj = system_checkEmail(SYSTEM_ACTIVE_ARTICLE)) {
						...
						system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_ACTIVE_ARTICLE);
					}
				}
			}
        ...
        <? if (!$isPopupApprove) { ?>
            <div id="top-content">
                <div id="header-content">
                    <h1><?=string_ucwords(system_showText(LANG_SITEMGR_ARTICLE))?> <?=system_showText(LANG_SITEMGR_STATUS)?></h1>
                </div>
            </div>
        <? } ?>
                ...
                <div class="baseForm">
                    ...
                    <? } else { ?>
                        ...
                        <form name="article_setting" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
                            ...
                            <button type="button" name="back" value="Back" class="input-button-form" onclick="<?=($isPopupApprove ? "parent.$.fancybox.close();" : "document.getElementById('formarticlesettingscancel').submit();")?>"><?=($isPopupApprove ? system_showText(LANG_SITEMGR_CANCEL) : system_showText(LANG_SITEMGR_BACK))?></button>

                        </form>

                        <form id="formarticlesettingscancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=ARTICLE_FEATURE_FOLDER;?>/<?=(($search_page) ? "search.php" : "index.php");?>" method="get">
