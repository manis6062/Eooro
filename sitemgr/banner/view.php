<?
	...
	if ($id) {
		...
		if ((!$banner->getNumber("id")) || ($banner->getNumber("id") <= 0)) {
			header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".BANNER_FEATURE_FOLDER."/index.php?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
			exit;
		}
	} else {
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".BANNER_FEATURE_FOLDER."/index.php?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
    ...
    <div id="content-content">
        ...
        <div class="default-margin">
            ...
            <ul class="list-view columnListView">
                <li>
                    <a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BANNER_FEATURE_FOLDER;?>/edit.php?id=<?=$banner->getNumber("id")?>&screen=<?=$screen?>&letter=<?=$letter?><?=(($url_search_params) ? "&$url_search_params" : "")?>" class="link-view">
                        <?=system_showText(LANG_SITEMGR_EDIT)?> <?=system_showText(LANG_SITEMGR_INFORMATION)?>
                    </a>
                </li>
                <li>
                    <a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BANNER_FEATURE_FOLDER;?>/settings.php?id=<?=$banner->getNumber("id")?>&screen=<?=$screen?>&letter=<?=$letter?><?=(($url_search_params) ? "&$url_search_params" : "")?>" class="link-view">
                        <?=string_ucwords(system_showText(LANG_SITEMGR_LABEL_CHANGE_BANNER_STATUS))?>
                    </a>
                </li>
                <li>
                    <a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BANNER_FEATURE_FOLDER;?>/delete.php?id=<?=$banner->getNumber("id")?>" class="link-view">
                ...
                <li>
                    ...
                    <? } else { ?>
                        <a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/account/view.php?id=<?=$banner->getNumber("account_id")?>" class="link-view"><?=system_showText(LANG_SITEMGR_LABEL_ACCOUNT)?></a>:
            ...
            <ul class="list-view columnListView secondaryListView">
                <li><span class="traffic"><a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BANNER_FEATURE_FOLDER;?>/report.php?id=<?=$banner->getNumber("id")?>&screen=<?=$screen?>&letter=<?=$letter?><?=(($url_search_params) ? "&$url_search_params" : "")?>"> <?=string_ucwords(system_showText(LANG_SITEMGR_TRAFFICREPORTS))?></a></span></li>
            ...
            <center>
                <a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BANNER_FEATURE_FOLDER;?>/preview.php?id=<?=$banner->getNumber("id")?>" class="standardLINK iframe fancy_window_preview_banner"><?=system_showText(LANG_SITEMGR_CLICKHERETOPREVIEW)?> <?=system_showText(LANG_SITEMGR_BANNER)?></a>
            </center>
