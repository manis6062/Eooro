<?
	...
	if ($id) {
		...
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".BANNER_FEATURE_FOLDER."/".(($search_page) ? "search.php" : "index.php")."");
		exit;
	}
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("bannersettings", $_POST, $message_bannersettings)) {
            ...
            if ($_POST['status'] == "S") {
                $bannerObj->setString("suspended_sitemgr", "y");
            } else {
                $bannerObj->setString("suspended_sitemgr", "n");
            }
			...
			if ($_POST['email_notification'] == 1 && $_POST['status'] == "A") {
				if ($bannerObj->getNumber("account_id") > 0) {
					...
					if ($emailNotificationObj = system_checkEmail(SYSTEM_ACTIVE_BANNER)) {
						...
						system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_ACTIVE_BANNER);
					}
				}
			}
			...
			$message = 2;
            if (!$isPopupApprove) {
                header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".BANNER_FEATURE_FOLDER."/".(($search_page) ? "search.php" : "index.php")."?message=$message&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
                exit;
            } else {
                ...
                <div class="baseForm">
                    ...
                    <? } else { ?>
                        ...
                        <form name="banner" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
                            ...
                            <button type="button" name="back" value="Back" class="input-button-form" onclick="<?=($isPopupApprove ? "parent.$.fancybox.close();" : "document.getElementById('formbannersettingscancel').submit();")?>"><?=($isPopupApprove ? system_showText(LANG_SITEMGR_CANCEL) : system_showText(LANG_SITEMGR_BACK))?></button>

                        </form>

                        <form id="formbannersettingscancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=BANNER_FEATURE_FOLDER;?>/<?=(($search_page) ? "search.php" : "index.php")?>" method="get">
