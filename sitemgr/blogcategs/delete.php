<?
	...
	if ($id) {
		...
		$fields = "`id`, `title`";
		$category = db_getFromDB("blogcategory", "id", $id, 1, "", "object", SELECTED_DOMAIN_ID, false, $fields);
		...
		$sql = "SELECT COUNT(0) as row_amount FROM Post WHERE id in ($search_post_ids)";
		...
		$message = 0;
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/blogcategs/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&category_id=".$category_id."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
		exit;
	}
