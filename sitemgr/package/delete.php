<?
	...
	if ($id) {
		...
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/package/index.php?message=".$message."&screen=$screen&letter=$letter");
		exit;
	}
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		$message = 2;
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/package/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
		exit;
	}
	...
	<div id="content-content">
		<div class="default-margin">
			...
			<div class="baseForm">
				...
				<form id="formpackagedeletecancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/package/<?=(($search_page) ? "search.php" : "index.php");?>" method="post">
					<input type="hidden" name="letter" value="<?=$letter?>" />
					<input type="hidden" name="screen" value="<?=$screen?>" />
					<?=system_getFormInputSearchParams((($_POST)?($_POST):($_GET)));?>
				</form>
