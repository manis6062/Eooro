<?
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("classifiedsettings", $_POST, $message_classifiedsettings)) {
            ...
            if ($_POST['status'] == "S") {
                $classifiedObj->setString("suspended_sitemgr", "y");
            } else {
                $classifiedObj->setString("suspended_sitemgr", "n");
            }
			...
			if ($_POST['email_notification'] == 1 && $_POST['status'] == "A") {
				if ($classifiedObj->getNumber("account_id") > 0) {
					...
					if ($emailNotificationObj = system_checkEmail(SYSTEM_ACTIVE_CLASSIFIED)) {
						...
						system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_ACTIVE_CLASSIFIED);
					}
				}
			}
        ...
        <? if (!$isPopupApprove) { ?>
            <div id="top-content">
                <div id="header-content">
                    <h1><?=string_ucwords(system_showText(LANG_SITEMGR_CLASSIFIED))?> <?=string_ucwords(system_showText(LANG_SITEMGR_STATUS))?></h1>
                </div>
            </div>
        <? } ?>
                ...
                <div class="baseForm">
                    ...
                    <? } else { ?>
                        ...
                        <form name="classified_setting" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
                            ...
                            <input type="hidden" name="module" value="classified" />
                            <button type="submit" value="Submit" class="input-button-form"><?=system_showText(LANG_SITEMGR_SUBMIT)?></button>
                            <button type="button" name="back" value="Back" class="input-button-form" onclick="<?=($isPopupApprove ? "parent.$.fancybox.close();" : "document.getElementById('formclassifiedsettingscancel').submit();")?>"><?=($isPopupApprove ? system_showText(LANG_SITEMGR_CANCEL) : system_showText(LANG_SITEMGR_BACK))?></button>

                        </form>

                        <form id="formclassifiedsettingscancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=CLASSIFIED_FEATURE_FOLDER;?>/<?=(($search_page) ? "search.php" : "index.php");?>" method="get">
