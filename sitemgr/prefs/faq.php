<?
		...
		if ($hiddenValue) {
			...
			if ($FAQ_post_submit) {
				...
				if ( validate_form("faq", $_POST, $error) ) {
					...
					$faqObj->Save();
					header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/prefs/faq.php?stat=0");
					exit;
				} else {
					header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/prefs/faq.php?stat=".$error);
					exit;
				}
	...
	$pageObj  = new pageBrowsing("FAQ", $screen, RESULTS_PER_PAGE, "id", "question", $letter, "editable='y'");
	$faqs = $pageObj->retrievePage("array");

	$paging_url = DEFAULT_URL."/".SITEMGR_ALIAS."/prefs/faq.php";
