<?
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("listingsettings", $_POST, $message_listingsettings)) {
            ...
            if ($_POST['status'] == "S") {
                $listingObj->setString("suspended_sitemgr", "y");
            } else {
                $listingObj->setString("suspended_sitemgr", "n");
            }
			...
			if ($_POST['email_notification'] == 1 && $_POST['status'] == "A") {
				if ($listingObj->getNumber("account_id") > 0) {
					...
					if($emailNotificationObj = system_checkEmail(SYSTEM_ACTIVE_LISTING)) {
						...
						system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_ACTIVE_LISTING);
					}
				}
			}
        ...
        <? if (!$isPopupApprove) { ?>
            <div id="top-content">
                <div id="header-content">
                    <h1><?=system_showText(LANG_SITEMGR_LISTING_SING)?> <?=string_ucwords(system_showText(LANG_SITEMGR_STATUS))?></h1>
                </div>
            </div>
        <? } ?>
                ...
                <div class="baseForm">
                    ...
                    <? } else { ?>
                        ...
                        <form name="listing_setting" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
                            ...
                            <button type="button" name="back" value="Back" class="input-button-form" onclick="<?=($isPopupApprove ? "parent.$.fancybox.close();" : "document.getElementById('formlistingsettingscancel').submit();")?>"><?=($isPopupApprove ? system_showText(LANG_SITEMGR_CANCEL) : system_showText(LANG_SITEMGR_BACK))?></button>

                        </form>

                        <form id="formlistingsettingscancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=LISTING_FEATURE_FOLDER;?>/<?=(($search_page) ? "search.php" : "index.php");?>" method="get">
