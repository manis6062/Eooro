...
<script type="text/javascript">
	function domainFinish (id) {
		...
		$.post(DEFAULT_URL + "/"+SITEMGR_ALIAS+"/domain/finish.php", {
	...
	function showPercentage (domain_id) {
		$.post(DEFAULT_URL + "/"+SITEMGR_ALIAS+"/domain/finish.php", {
			...
			if (aRet[1] >= 0 && aRet[1] < 100) {
				...
				window.location = DEFAULT_URL + "/"+SITEMGR_ALIAS+"/domain/index.php?message=0&domain_id=" + domain_id;
		...
		<div class="default-margin">
			...
			<form name="domain" id="domain" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
				...
				<div id="domainButtons">
					...
					<button type="button" name="cancel" class="input-button-form" value="Cancel" onclick="window.location = '<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/domain/index.php<?=$id? "?cancel=".$id :''?>'"><?=system_showText(LANG_SITEMGR_CANCEL)?></button>
				</div>
			</form>
