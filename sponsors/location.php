<?
	...
	$type = $_GET["type"];
        ...
        if($level == '1'){
            ...
            $select_location_msg = system_showText(constant("LANG_SEARCH_LABELCBSTATE"));
        }
        else if($level == '3'){
            $select_location_msg = system_showText(constant("LANG_SEARCH_LABELCBCITY"));
        }
	if ($type == 'byId') {
		if ($childLevel) {
			...
			if ($retrieved_locations) {
				$return = "<option id=\"l_location_".$childLevel."\" value=\"\">$select_location_msg</option>";
		...
		if ($retrieved_locations) {
			$return = "<option id=\"l_location_".$level."\" value=\"\">$select_location_msg</option>";
			foreach ($retrieved_locations as $each_location) {
				$location_id = $each_location['id'];
				$location_name = $each_location['name'];
