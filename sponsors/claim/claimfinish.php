	...
	if (!$claim_approve) {
		...
		if ($claim_approveemail) {
			...
			if ($emailNotificationObj = system_checkEmail(SYSTEM_CLAIM_AUTOMATICALLY_APPROVED)) {
				...
				$error = false;
				system_mail($contact->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", $claimlistingid, $contact->account_id, SYSTEM_CLAIM_AUTOMATICALLY_APPROVED);
			}
		}
	}
