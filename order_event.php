<?
	...
	if (!in_array($level, $evLevelValue)) {
		header("Location: ".DEFAULT_URL."/".ALIAS_ADVERTISE_URL_DIVISOR.".php?event");
		exit;
	}
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($validate_account && $validate_contact && $validate_event && $validate_discount) {
			...
			if ($emailNotificationObj = system_checkEmail(SYSTEM_EVENT_SIGNUP)) {
				...
				system_mail($contact->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contact->account_id, SYSTEM_EVENT_SIGNUP);
			}
			////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            $sitemgr_msg .= "<br /><a href=\"".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/".EVENT_FEATURE_FOLDER."/view.php?id=".$event->getNumber("id")."\" target=\"_blank\">".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/".EVENT_FEATURE_FOLDER."/view.php?id=".$event->getNumber("id")."</a><br /><br />";
