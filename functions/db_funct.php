...
function db_ArrayDomainInfo() {
    ...
    DBQuery::execute(function() use ($main) {
        $sql = $main->prepare("SELECT smaccount_id,
					   name,
					   database_host,
					   database_port,
					   database_username,
...
function db_getFromDBXML_pdo($table, $by_key = "", $by_value = "", $number = 1, $orderby = "", $fields, $sql = false, $domain_id = SELECTED_DOMAIN_ID) {
    $domain = DBConnection::getInstance()->getDomain();
    return DBQuery::execute(function() use ($domain, $table, $by_key,$by_value,$number, $orderby, $fields, $sql,$domain_id) {
                    ...
                    if (is_array($by_key) && is_array($by_value)) {
                        ...
                        if (count($by_key) == count($by_value) && isset($by_key[0]) && isset($by_value[0])) {
                            for ($i = 0; $i < count($by_key); $i++) {
                                $where[] .= "$by_key[$i] = :$by_key[$i]";
                                $parameters[':'.$by_key[$i]] = $by_value[$i];
...
function db_getFromDB_pdo($table, $by_key = "", $by_value = "", $number = 1, $orderby = "", $return = "object", $domain_id = false, $package = false, $fields = "*", $where = "", $debug = false) {
    
    return DBQuery::execute(function() use ($table, $by_key, $by_value, $number, $orderby,$return, $domain_id, $package, $fields, $where, $debug) {
        switch ($table) {
            case 'account' : $obj = "Account";
                break;
            case 'smaccount' : $obj = "SMAccount";
                break;
            case 'contact' : $obj = "Contact";
                break;
            case 'listing' : $obj = "Listing";
                break;
            case 'gallery' : $obj = "Gallery";
                break;
            case 'promotion' : $obj = "Promotion";
                break;
            case 'listingcategory' : $obj = "ListingCategory";
                break;
            case 'classifiedcategory' : $obj = "ClassifiedCategory";
                break;
            case 'classified' : $obj = "Classified";
                break;
            case 'articlecategory' : $obj = "ArticleCategory";
                break;
            case 'article' : $obj = "Article";
                break;
            case 'eventcategory' : $obj = "EventCategory";
                break;
            case 'event' : $obj = "Event";
                break;
            case 'banner' : $obj = "Banner";
                break;
            case 'invoice' : $obj = "Invoice";
                break;
            case 'editor_choice' : $obj = "Editor_Choice";
                break;
            case 'listing_choice' : $obj = "Listing_Choice";
                break;
            case 'payment_log' : $obj = "Payment_Log";
                break;
            case 'custominvoice' : $obj = "CustomInvoice";
                break;
            case 'listingtemplate' : $obj = "ListingTemplate";
                break;
            case 'location1' : $obj = "Location_1";
                break;
            case 'location2' : $obj = "Location_2";
                break;
            case 'location3' : $obj = "Location_3";
                break;
            case 'location4' : $obj = "Location_4";
                break;
            case 'location5' : $obj = "Location_5";
                break;
            case 'settinglocation' : $obj = "Setting_Location";
                break;
            ...
            case 'blog' : $obj = "Post";
                break;
            case 'appnotification' : $obj = "AppNotification";
                break;
            case 'appadvert' : $obj = "AppAdvert";
                break;
            case 'case' : $obj = 'Opened_Cases';
                break;
        }
...
//            $db = db_getDBObjectByDomainID($domain_id, $dbMain);
             ...
             */
            $db = db_checkTableMainDomain_pdo($table);
...
function db_getFromDBBySQL_pdo($table, $sql, $return = "object", $forceDB = false, $domain_id = 0) {
    return DBQuery::execute(function() use ($table, $sql, $reutrn, $forceDB, $orderby,$return, $domain_id) {
    switch ($table) {
        case 'account' : $obj = "Account";
            break;
        case 'smaccount' : $obj = "SMAccount";
        ...
        case 'location5' : $obj = "Location_5";
            break;
        case 'comment' : $obj = "Comments";
            break;
        case 'post' : $obj = "Post";
            break;
        case 'cron_log' : $obj = "Cron_Log";
            break;
        default : $obj = $table;
            break;
    }
    ...
    $dbObjAux = DBConnection::getInstance()->getMain();
    ...
    if ($return == "array") {
        $sql = $db->prepare($sql,array(PDO::MYSQL_ATTR_USE_BUFFERED_QUERY => false));
...
function db_getFromDB($table, $by_key="", $by_value="", $number=1, $orderby="", $return="object", $domain_id = false, $package = false, $fields = "*", $where = "", $debug = false) {
		...
		switch ($table) {
			...
			case 'appadvert'            : $obj = "AppAdvert";               break;
                        case 'case'             : $obj = 'Opened_Cases'; break;
    ...
    if ($dbObj_main && ($domain_id || $url)) {
        ...
        if (is_array($array_domains) && ($array_domains["status"] == "A" || $array_domains["status"] == "P")) {
            ...
            DBQuery::execute(function() use ($main) {
                ...
                if ($url) {
                    $sql = $main->prepare("SELECT database_host, database_port, database_username, database_password, database_name FROM Domain WHERE url = :url AND (`status` =:status  OR `status` = :status1)");
                    $parameters = array(
                    ...
                    $sql = $main->prepare("SELECT database_host, database_port, database_username, database_password, database_name FROM Domain WHERE id = :id AND (`status` = :status OR `status` = :status1)");
                    $parameters = array(
                        ':id'=>$domain_id,
                        ':status'=>'A',
                        ':status1'=>'P'
                ...
                //$result = $dbObj_main->query($sql);
                if ($rows>0) {
                    $array_domains = $sql->fetch(\PDO::FETCH_ASSOC);
...
function db_getFields_pdo($db, $table, $use_domain = false) {
    return DBQuery::execute(function() use ($db,$table, $use_domain) {
    $sql = "desc " . $table;
    $sql = $db->prepare($sql);
    $sql->execute();
    $rows = $sql->rowCount();
...
function db_checkTableMainDomain_pdo($table) { 
    /*
     * Force connection with main DB
     */
    $array_tables_main_table[] = "account";
