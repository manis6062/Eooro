  ...
  <form name="rate_form" id="1form">
    <div class="row">
      <div class="col-sm-12">
          ...
          <div class="form-group formimage reviewmbtm">
              <input type="text" name="twitter" class="form-control loginform reviewinput reviewplacehld" id="twitter" placeholder="Twitter Feed" value="<?=$twitter?>" maxlength="100" tabindex="5">
          </div>
          ...
          <div class="row captchawrapper captcha">
              <div class="col-lg-3 col-sm-3 col-xs-12 one">
                  <img id="capimg" src="<?=DEFAULT_URL?>/includes/code/captcha.php" border="0" alt="<?=system_showText(LANG_CAPTCHA_ALT)?>" title="<?=system_showText(LANG_CAPTCHA_TITLE)?>" height="48px" />*
              </div>
              <div class="col-lg-2 col-sm-2 col-xs-12">
                  <input type="text" value="" name="captcha" class="form-control loginform login-form2" id="captcha" tabindex="11">
              </div>
          </div> <!--/row--> 

          <div class="action">
            <div class="row col-sm-4">
                <input type="submit" name="submit" value="Add" id="submitReview" class="btn btn-default btn-lg reviewsendbtn reviewsendbtn1" tabindex="12">
            </div>
          </div>       
      </div> <!-- col-sm-12 -->
    </div><!-- row -->
    ...
    // Setup form validation on the #register-form element
        ...
        messages: {
            ...
            "street_address": {
                ...
                minlength: "Street address should be atleast of 5 characters",
            },
            is_owner  : "*",
          ...
          var captcha = $("#captcha").val();
            $.ajax({
              method: "POST",
              url: "<?=DEFAULT_URL.'/'.MEMBERS_ALIAS.'/listing/check.php'?>",
              data: { captcha_value: captcha }
            })
              .done(function( msg ) {
                if(msg.indexOf("invalid") > -1){
                  var date = new Date();
                  $("#capimg").attr("src", "<?=DEFAULT_URL?>/includes/code/captcha.php?"+date.getTime());
                  $("#captcha").val('');
                  $('#submitReview').click();
                } else {
                  $('.resultsMessage').hide();
