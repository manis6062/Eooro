			...
			if (mysql_num_rows($result) > 0){
				...
				while ($row = mysql_fetch_assoc($result)) {
					...
					if ($contactObj->getString("email")) {
						...
						$table = report_PrepareListingStatsReviewToEmail($data_email, $row["id"], "listing", $non_secure_url);
						...
						$error = false;
                        if ($table) {
                            system_mail($to, $subject, $message, $from_name." <".$from_email.">", $content_type, '', '', $error, "", "", "", "", $contactObj->account_id, SYSTEM_EMAIL_TRAFFIC);
                            if ($bcc) {
                                system_mail($bcc, $subject, $message, $from_name." <".$from_email.">", $content_type, '', '', $error, "", "", "", "", $contactObj->account_id, SYSTEM_EMAIL_TRAFFIC);
                            }
                        }
