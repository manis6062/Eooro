	...
	if (mysql_num_rows($resIL) > 0) {
			...
			while ($line = fgetcsv($handle, 16384, $delimiter)) {
				...
				if (count($line) > count($sample_header)) {
					...
					for ($i=0; $i<count($sample_header); $i++) {
						...
						if ($i == 0 || $i == 1 || $i == 16 || $i == 21 || $i == 23 || $i == 25 || $i == 27 || $i == 29 || $i == 32 || $i == 33 || $i == 38 || $i == 39) {
							...
							if ($i == 16){ //validate listing title
								if (!$line[$i]) {
		...
		if ($import_stop) {
			...
			$sitemgr_msg = system_showText(LANG_LABEL_SITE_MANAGER).",<br /><br />
							...
							$sitemgr_msg .= "<br /><a href=\"http://".$default_url."/".SITEMGR_ALIAS."/import/importlog.php?log_id=".$importlogObj->getNumber("id")."\" target=\"_blank\">http://".$default_url."/".SITEMGR_ALIAS."/import/importlog.php?log_id=".$importlogObj->getNumber("id")."</a><br /><br />";
