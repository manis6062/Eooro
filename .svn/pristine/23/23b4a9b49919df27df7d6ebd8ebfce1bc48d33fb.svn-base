<?
	...
	if ($_GET["url_full"] && (string_strpos($_GET["url_full"], ALIAS_PROMOTION_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false || string_strpos($_GET["url_full"], ALIAS_PROMOTION_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false || string_strpos($_GET["url_full"], ALIAS_PROMOTION_MODULE."/".ALIAS_REVIEW_URL_DIVISOR) !== false)) {

		$url = string_replace_once(EDIRECTORY_FOLDER."/".ALIAS_PROMOTION_MODULE."/", "", $_GET["url_full"]);
		$parts = explode("/", $url);

		if (string_strpos($_GET["url_full"], ALIAS_PROMOTION_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false){
		...
		} else if (string_strpos($_GET["url_full"], ALIAS_PROMOTION_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false){
			$browsebylocation = true;

			for ($i = 1; $i<count($parts); $i++){
				$_GET["friendLoc".$i] = $parts[$i];
			}
		} else if (string_strpos($_GET["url_full"], ALIAS_PROMOTION_MODULE."/".ALIAS_REVIEW_URL_DIVISOR) !== false) {
			$browsebyitem = true;
	 ...
	 */
	if ($auxFullFriendlyUrl && string_strpos($_GET["url_full"], ALIAS_PROMOTION_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR)) {
