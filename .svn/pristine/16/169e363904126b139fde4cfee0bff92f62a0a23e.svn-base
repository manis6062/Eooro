	...
	if ($allNot && (count($allNot) > 0)) {
		...
		while ($row = mysql_fetch_assoc($result)) {
			...
			if ($reminder && $contactObj->getString("email")) {
				...
				$error = false;
				system_mail($to, $subject, $message, $from_name." <".$from_email.">", $content_type, '', '', $error, "", "", "", "", $contactObj->account_id);
				if ($bcc) {
					system_mail($bcc, $subject, $message, $from_name." <".$from_email.">", $content_type, '', '', $error, "", "", "", "", $contactObj->account_id);
				}
