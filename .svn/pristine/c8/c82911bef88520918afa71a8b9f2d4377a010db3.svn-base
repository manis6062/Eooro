<?php 
	...
	if(Braintree_NotificationLog::save($webhookNotification, $subscription)) {
        ...
        ############### listing update ###########################################
        $listingObj->renewal_date = date("Y-m-d"); 
        $listingObj->custom_text2 = "";         // Subscription ID for braintree
        $listingObj->custom_text3 = "";         // nextBillingDate from braintree
        $listingObj->custom_text4 = "";         // customer id for braintree
        $listingObj->account_id   = 0;
