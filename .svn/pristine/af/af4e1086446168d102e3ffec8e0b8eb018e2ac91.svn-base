...
function loadCategoryTree(action, prefix, category, category_id, template_id, path, domain_id) {
	...
	if (xmlhttp) {
		...
		xmlhttp.open("GET", path + "/"+SITEMGR_ALIAS+"/loaddisabledcategorytree.php?action=" + action + "&prefix=" + prefix + "&category=" + category + "&category_id=" + category_id + "&template_id=" + template_id + "&ajax_categories=" + ajax_categories + "&domain_id=" + domain_id, true);
		xmlhttp.send(null);
	}
}
