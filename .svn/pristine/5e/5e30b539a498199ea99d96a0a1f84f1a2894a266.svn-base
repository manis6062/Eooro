<?
	...
	if ($id) {
		...
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".ARTICLE_FEATURE_FOLDER."/index.php?message=".$message."&screen=$screen&letter=$letter");
		exit;
	}
    ...
    <div id="content-content">
        <div class="default-margin">
            ...
            <? }else { ?>
                ...
                <form id="formarticledeletecancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=ARTICLE_FEATURE_FOLDER;?>/<?=(($search_page) ? "search.php" : "index.php");?>" method="post">
                    <input type="hidden" name="letter" value="<?=$letter?>" />
                    <input type="hidden" name="screen" value="<?=$screen?>" />
                    <?=system_getFormInputSearchParams((($_POST)?($_POST):($_GET)));?>
                </form>
