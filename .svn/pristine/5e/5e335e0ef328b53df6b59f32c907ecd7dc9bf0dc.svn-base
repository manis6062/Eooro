...
?>
	...
	<div id="importInfo_0" <?=$_GET["type"] == "event" ? "style=\"display:none\"" : ""?>>
		<div class="header-form"><?=system_showText(LANG_SITEMGR_DEFAULTSETTINGS." - ".LANG_LISTING_FEATURE_NAME_PLURAL)?></div>
		...
		<table cellpadding="2" cellspacing="0" class="table-form table-form-settings table-form-margin">
            ...
            <tr>
				<td align="right" class="td-form"><div class="label-form"><?=system_showText(LANG_SITEMGR_SETTINGS_IMPORT_FRIENDLYURL)?></div></td>
				<td align="left" class="td-form">
					<input type="checkbox" id="import_update_friendlyurl" name="import_update_friendlyurl" value="1" align="absmiddle" <?=$import_update_friendlyurl?> style="width: auto; border: 0;" class="inputCheck" />
				</td>
			</tr>
	...
	<? if (EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on") {?>
		...
		<div id="importInfo_1" <?=$_GET["type"] == "event" ? "" : "style=\"display:none\""?>>
			<div class="header-form"><?=system_showText(LANG_SITEMGR_DEFAULTSETTINGS." - ".LANG_EVENT_FEATURE_NAME_PLURAL)?></div>
