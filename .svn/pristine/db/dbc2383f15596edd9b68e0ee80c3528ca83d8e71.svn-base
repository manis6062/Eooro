<?
	...
	if ($claim_denyemail) {
		...
		if ($emailNotificationObj = system_checkEmail(SYSTEM_CLAIM_DENIED)) {
			...
			system_mail($contact->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", $listing->id, $contact->account_id, SYSTEM_CLAIM_DENIED);
		}
	}

	$message = 0;
    header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/claim/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : ""));
	exit;
