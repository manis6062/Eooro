<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/forms/form_comment.php
	# ----------------------------------------------------------------------------------------------------

?>

    <div class="contact-form flex-box-group color-1">

        <h2><?=system_showText(LANG_MSG_LEAVEYOURCOMMENT)?></h2>

        <form class="form" name="commentForm" id="commentForm" method="post" action="<?=system_getFormAction($_SERVER["REQUEST_URI"])?>">
            ...
            <p id="error" class="errorMessage" <?=(!$message_comment ? "style=\"display:none\"" : "")?>>
                <?=$error_comment;?>
            </p>
            ...
            <div>
                ...
                <input class="text" type="email" name="comment_email" id="comment_email" value="<?=$comment_email?>" maxlength="100" tabindex="2" />
            </div>
            ...
            <? } else {
                echo "<p class=\"informationMessage\"><a rel=\"nofollow\" href=\"".DEFAULT_URL."/popup/popup.php?pop_type=profile_login&amp;destiny=$destiny\" class=\"fancy_window_iframe\">".LANG_MSG_NEED_LOGIN."</a></p>";
            } ?>
        </form>
