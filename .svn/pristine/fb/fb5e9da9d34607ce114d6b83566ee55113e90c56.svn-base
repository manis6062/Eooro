...
//ini_set( 'display_errors', 'on' );
    ...
    $business_title   = htmlspecialchars_decode(ucfirst($title));
...
<?}else{?>
    <meta itemprop="og:description" content="<?=$content?>"/>
<?}?>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        ...
        if ((strpos($_SERVER['REQUEST_URI'], SOCIALNETWORK_FEATURE_NAME) > -1)
                ...
                elseif((strpos($_SERVER['REQUEST_URI'], 'advertise'))
                        || (strpos($_SERVER['REQUEST_URI'], 'company-reviews'))
...
<? if(ACTUAL_PAGE_NAME == 'ACTUAL_PAGE_NAME' && !strpos($_SERVER['REQUEST_URI'], "results.php") && !strpos($_SERVER['REQUEST_URI'], "termsofuse.php") && !strpos($_SERVER['REQUEST_URI'], "privacypolicy.php")&& !strpos($_SERVER['REQUEST_URI'], "order_listing.php")) {?>
