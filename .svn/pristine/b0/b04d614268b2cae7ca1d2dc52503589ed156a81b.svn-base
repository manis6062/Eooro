<?
	...
	*/     	
	function cachefull_postprocessing($marker, $cachedContent) {
		...
		if (strpos ($cachedContent, "<!--{$marker}-->") !== false) {
			...
			switch ($marker) {
                ...
                case "cachemarkerNewsletter":
                    include(system_getFrontendPath("newsletter.php", "frontend"));
                break;
            
                case "cachemarkerTopItems":
                    include(system_getFrontendPath("top_items.php"));
                break;
            
                case "cachemarkerEventCalendar":
			...
			$cachedNewContent = preg_replace($regexPattern, $marker, $cachedContent);
			
            if (!is_null($cacheNewContent)) {
                $cachedContent = str_replace($marker, $freshNewContent, $cachedNewContent);
            } else {
                $oldWorkaroundContent = substr($cachedContent, strpos($cachedContent, "<!--{$marker}-->"), strrpos($cachedContent, "<!--{$marker}-->") - strpos($cachedContent, "<!--{$marker}-->") + strlen($marker)+7); 
                $cachedContent = str_replace($oldWorkaroundContent, $freshNewContent, $cachedContent);
            }
    ...
    /**
	* Define constants to be used to save cache file according to the URL
	* @copyright Copyright 2014Arca Solutions, Inc.
	* @author Arca Solutions, Inc.
	* @name void cachefull_prepareFileName()
	* @return void
	*/ 
    function cachefull_prepareFileName() {
        
        $auxFileName = $_SERVER["REQUEST_URI"];
        ...
        if (
            ...
            if (ACTUAL_MODULE_FOLDER == LISTING_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["listing_results_per_page"];
            } elseif (ACTUAL_MODULE_FOLDER == EVENT_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["event_results_per_page"];
            } elseif (ACTUAL_MODULE_FOLDER == ARTICLE_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["article_results_per_page"];
            } elseif (ACTUAL_MODULE_FOLDER == CLASSIFIED_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["classified_results_per_page"];
            } elseif (ACTUAL_MODULE_FOLDER == PROMOTION_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["promotion_results_per_page"];
            } elseif (ACTUAL_MODULE_FOLDER == BLOG_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["blog_results_per_page"];
            }
            
        } elseif (string_strpos($_SERVER["REQUEST_URI"], ALIAS_REVIEW_URL_DIVISOR."/") !== false) {
            
            if (ACTUAL_MODULE_FOLDER == LISTING_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["listingreviews_results_per_page"];
            } elseif (ACTUAL_MODULE_FOLDER == ARTICLE_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["articlereviews_results_per_page"];
            } elseif (ACTUAL_MODULE_FOLDER == PROMOTION_FEATURE_FOLDER) {
                $auxFileName .= "&per_page=".$_COOKIE["dealreviews_results_per_page"];
            }            
            
        } elseif (string_strpos($_SERVER["REQUEST_URI"], ALIAS_CHECKIN_URL_DIVISOR."/") !== false) {
            
            $auxFileName .= "&per_page=".$_COOKIE["checkin_results_per_page"];
            
        } elseif (string_strpos($_SERVER["REQUEST_URI"], ALIAS_LISTING_ALLCATEGORIES_URL_DIVISOR."/") !== false) {
            
            $auxFileName .= "&per_page=".$_COOKIE["listingcategory_results_per_page"];
            
        } elseif (string_strpos($_SERVER["REQUEST_URI"], ALIAS_BESTOF_URL_DIVISOR."/") !== false) {
            
            $auxFileName .= "&per_page=".$_COOKIE["bestof_results_per_page"];
        ...
        if ($isMobile == "y") {
            ...
            $auxFileName .= $auxDevice;
        }

        define("CACHE_FULL_FILE_NAME", md5($_SERVER["HTTP_HOST"].(($_SERVER["REQUEST_URI"] == "/") ? "/index.php$auxDevice" : $auxFileName)));
            ...
            if (@file_exists (CACHE_FULL_UPDATETOKEN)) {
                ...
                if ($cacheFileSize) {
                    ...
                    if (CACHE_FULL_ALWAYS_FRESH_FEATURED_DEAL == "on") {
                        ...
                        $cachedContent = cachefull_postprocessing("cachemarkerFeaturedDeal3", $cachedContent);
                    }
                    ...
                    $cachedContent = cachefull_postprocessing("cachemarkerNewsletter", $cachedContent);

                    $cachedContent = cachefull_postprocessing("cachemarkerTopItems", $cachedContent);

                    $cachedContent = cachefull_postprocessing("cachemarkerEventCalendar", $cachedContent);
	...
	*/ 	
	function cachefull_footer() {
		...
		if (cachefull_abletorun()) {
            ...
            is_ie(false, $ieVersion);

            if (CACHE_FULL_INCLUDE_CACHE_COMMENT_AT_PAGE == "on" && $ieVersion != 7) {
		...
		if (cachefull_abletorun()) {
			...
			if (! @file_exists (CACHE_FULL_UPDATETOKEN)) {
				...
				(	
                    ...
                    || (strpos($query, "delete from faq") !== false)
				)
				{
	...
	*/ 		
	...
	function cachepartial_mysqlupdater($query) {
		...
		if((stripos ($query, "insert") === 0) || (stripos ($query, "update") === 0)) {
			...
			} elseif ( strpos($query, 'blogcategory') !== false ) {
		...
		if (is_array($files)) {
			foreach ($files as $file) {
				if ( strpos($file, '(.*)') === false ) {
                    $_file = CACHE_PARTIAL_DIR.rtrim($file, '.php').'.php';
   ...
   /**
	* This function will clean up CACHE_FILTER_FOLDER directory 
	* @copyright Copyright 2011 Arca Solutions, Inc.
	* @author Arca Solutions, Inc.
	* @name void cachefilter_cleanup(void)
	* @return void
	*/ 	
	function cachefilter_cleanup($query) {
        ...
        if ((stripos ($query, "insert") === 0) || (stripos ($query, "update") === 0) || (stripos ($query, "delete") === 0)) {
            ...
            if ( strpos($query, 'level') !== false ) {
                $enable_to_delete = true;
            }
            if ( strpos($query, 'setting') !== false ) {
                $enable_to_delete = true;
            }
            
            if ($enable_to_delete) {
                $cacheRelativeDirectoryHandler = @opendir (CACHE_FILTER_FOLDER);
                while ($cacheFiles = readdir ($cacheRelativeDirectoryHandler)) {
                    if ($cacheFiles != '.' && $cacheFiles != '..') {
                        @unlink(CACHE_FILTER_FOLDER . "/" . $cacheFiles);
                    }
                }
                @closedir ($cacheRelativeDirectoryHandler);
