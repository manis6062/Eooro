	...
	if ($delete == "true") {
		...
		if ($type != "banner") {
			...
			if ($type == "Listing") {
                ...
                $sql = "SELECT category_id, post_id FROM Blog_Category WHERE post_id IN (".$ids.") ORDER BY post_id";
                
                $idsT = explode(",",$ids);
				$catEmpty = false;
				foreach ($idsT as $idT){
					$sqlT = "SELECT category_id, post_id FROM Blog_Category WHERE post_id = $idT ORDER BY post_id";
					$resultT = $dbObj->query($sqlT);
					$rowT = mysql_fetch_assoc($resultT);
					if (!$rowT) $catEmpty = true;
				}
			...
			if($type == "Listing"){
				...
				/**
				 * Get categories for last Post_Id
				 */
				$array_category[$i] = $aux_array_categories;
			...
			$strCatId = "";
            if ($type == "Blog") {
                $maxCategs = count($array_categories);
            } elseif ($type == "Listing") {
                $maxCategs = LISTING_MAX_CATEGORY_ALLOWED;
            } else {
                $maxCategs = MAX_CATEGORY_ALLOWED;
            }
			for ($i = 1; $i <= $maxCategs; $i++) {
				if($type == "Listing"){
