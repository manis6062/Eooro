	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("article", $_POST, $message_article) && is_valid_discount_code($_POST["discount_id"], "article", $_POST["id"], $message_article, $discount_error_num)) {
			...
			***************************************************************************************************/
			...
			if ($article->getNumber("account_id") > 0) {
				if($message == 0) {
					...
					if($emailNotificationObj = system_checkEmail(SYSTEM_NEW_ARTICLE)) {
						...
						$error = false;
						system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_NEW_ARTICLE);
					}
				}
			}
			...
			header("Location: $url_redirect/".(($search_page) ? "search.php" : "index.php")."?module=article&process=".$process."&newest=".$newest."&message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : ""));
			exit;
