<?
	...
	if ($delete && $id) {
		...
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/prefs/editorchoice.php");
		exit;
	}
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if ($delete) {
			...
			$editorChoice->delete();
			$message = 0;
			header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/prefs/editorchoice.php?message=".$message."");
			exit;
		} else {
		...
		<div class="default-margin">
			...
			<? if ($delete) { ?>
				...
				<div class="baseForm">
					...
					<form name="account" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
						...
						<p class="informationMessage">
							...
							if ($imageObj->imageExists()) {
								echo $imageObj->getTag(true, IMAGE_DESIGNATION_WIDTH, IMAGE_DESIGNATION_HEIGHT, $editorChoice->getString("name", false))."<br />";
							}
							?>
					...
					<form id="formeditorchoicecancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/prefs/editorchoice.php" method="post">
					</form>
