			...
			foreach ($locations as $location) {
				...
				<tr>
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <a href="<?=$url_redirect?>/delete.php?<?=($_location_node_params?$_location_node_params."&":"")?>id=<?=$id?>" class="link-table">
                                            <?=system_showText(string_ucwords(constant("LANG_SITEMGR_LOCATION_DELETE".constant("LOCATION".$_location_level."_SYSTEM"))))?>
                                        </a>
                                    </li>
					...
					<td nowrap class="main-options text-center">
						...
						<? if ($location->getNumber('id') > 0) { ?>
							<? if ($_location_child_level!==false) { ?>
								<a href="<?=$url_base?>/locations/location_<?=$_location_child_level?>/index.php?<?=($_location_node_params?$_location_node_params."&":"")?>location_<?=$_location_level?>=<?=$id?>">
									<?=system_showText(string_ucwords(constant("LANG_SITEMGR_LOCATION_VIEW".constant("LOCATION".$_location_child_level."_SYSTEM"))))?>
								</a>
                                <b>|</b>
							<? } ?>

							<a href="<?=$url_redirect?>/location_<?=($_location_level)?>.php?<?=($_location_node_params?$_location_node_params."&":"")?>id=<?=$id?>&operation=edit">
	...
	if ($_location_father_level !== false) {
		if (isset(${"location_".$_location_father_level}) && ${"location_".$_location_father_level} != "") { ?>
			<p class="informationMessage">
				...
				<a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/locations/location_<?=$_location_level?>/location_<?=$_location_level?>.php?<?=($_location_node_params?$_location_node_params."&":"")?>operation=add">
