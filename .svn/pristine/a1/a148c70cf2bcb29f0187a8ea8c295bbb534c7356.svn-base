	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if ($_POST["temp"] == 'y'){
			...
			if ($_POST["gallery_image_id"]){
                ...
                if ($item_type == "Blog"){
                    $item_type = "Post";
                    $auxSql = ", image_caption = '' ";
                    $auxSql .= ", thumb_caption = '' ";
                } else {
                    $auxSql = "";
                }
				...
				$dbObjMain = db_getDBObject(DEFAULT_DB, true);
				$dbObj = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID, $dbObjMain);
				$sql = "UPDATE $item_type SET image_id = '', thumb_id = '' $auxSql WHERE id = ".$_POST["item_id"];
				$dbObj->query($sql);
			}
		}
	...
	<head>
		...
		<?if ($deleteImageSucess == "success"){?>
			<script language="javascrip" type="text/javascript">
				parent.loadGallery(<?=$_POST["item_id"]?>, 'y', '<?=SITEMGR_ALIAS?>');
				setTimeout(function(){
