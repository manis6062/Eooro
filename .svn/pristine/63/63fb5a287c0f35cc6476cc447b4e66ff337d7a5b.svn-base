<?
    ...
    function check_featuredcategory() {
        if (document.getElementById("featuredcategory_enabled").checked) {
            ...
            ':'').'
            '.(BLOG_FEATURE == "on" && CUSTOM_BLOG_FEATURE == "on"? '
            document.getElementById("blog_featuredcategory_enabled").disabled = false;
            document.getElementById("blog_featuredcategory_enabled").checked = true;
            ':'').'
            ...
            ':'').'	
            '.(BLOG_FEATURE == "on" && CUSTOM_BLOG_FEATURE == "on" ? '
            document.getElementById("blog_featuredcategory_enabled").disabled = true;
            document.getElementById("blog_featuredcategory_enabled").checked = false;
            ':'').'
        }
    }
    ...
    function verifyExistsPopCateg() {	
        ...
        '.(BLOG_FEATURE == "on" && CUSTOM_BLOG_FEATURE == "on" ? 'flag_blog = $("#blog_featuredcategory_enabled").is(":checked");':'').'
        if (!(flag_listing'.(EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on" ? '||flag_event':'').''.(CLASSIFIED_FEATURE == "on" && CUSTOM_CLASSIFIED_FEATURE == "on" ? '||flag_classified':'').''.(ARTICLE_FEATURE == "on" && CUSTOM_ARTICLE_FEATURE == "on" ? '||flag_article':'').''.(BLOG_FEATURE == "on" && CUSTOM_BLOG_FEATURE == "on" ? '||flag_blog':'').')) {
    ...
    <table cellpadding="2" cellspacing="0" border="0" class="table-form">
        ...
        <? if (BLOG_FEATURE == "on" && CUSTOM_BLOG_FEATURE == "on") { ?>
            <tr class="tr-form">
                <td align="left" class="td-form">
                    <input type="checkbox" name="blog_featuredcategory_enabled" id="blog_featuredcategory_enabled" value="on" <?=$blog_featuredcategory_enabled_checked?>  class="inputCheck" onclick="verifyExistsPopCateg();" />
                </td>
                <td>
                    <div class="label-form" align="left"><label for="blog_featuredcategory_enabled"><?=system_showText(string_ucwords(LANG_SITEMGR_BLOG))?></label></div>
                </td>
            </tr>
        <? } ?>
