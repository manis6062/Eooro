<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST") && (!DEMO_LIVE_MODE)) {
		...
		if (($submit_button || $last_crop) && (!DEMO_LIVE_MODE)) {
			/* setting title */
			if (!customtext_set("header_title", $header_title)) {
				if (!customtext_new("header_title", $header_title)) {
					$error = true;
					$actions = "";
				}
			}
            if (!$error) {
                ...
                $constValues = array();
                $constValues["name"] = ($header_title ? $header_title : EDIRECTORY_TITLE);
			...
			/* setting author */
			if (!customtext_set("header_author", $header_author)) {
				if (!customtext_new("header_author", $header_author)) {
					$error = true;
					$actions = "";
				}
			}
			...
			/* setting description */
			if (!customtext_set("header_description", $header_description)) {
				if (!customtext_new("header_description", $header_description)) {
					$error = true;
				}
			}

			/* setting keywords */
			if (!customtext_set("header_keywords", $header_keywords)) {
				if (!customtext_new("header_keywords", $header_keywords)) {
					$error = true;
				}
			}
			...
			if($actions) {
                ...
                todo_updateItem("todo_headerlogo");
			}
