<?
	...
	if ($user) {
            ...
            if ($members) {
                if (($id == sess_getAccountIdFromSession()) || ($members != "profile")) {
                    ...
                    $removeFavoritesDiv = " <div class=\"coverFavorites boxFavorites\">
                                                <span><a rel=\"nofollow\" id=\"favoritesRemove_".$event->getNumber("id")."\" href=\"".$remove_favorites_link."\" ".$remove_favorites_click." ".$remove_favorites_style.">".system_showText(LANG_ICONQUICKLIST_REMOVE)."<img src=\"".DEFAULT_URL."/images/bt_delete.gif\" border=\"0\" alt=\"\" title=\"\"/></a></span>
                                            </div>";
                }
            }
		}
		...
		if (SOCIAL_BOOKMARKING == "on") {
			...
			if ($eventLevelObj->getDetail($icon_event_level) == "y") {
                ...
                $sbmLinkShare = EVENT_DEFAULT_URL."/".ALIAS_SHARE_URL_DIVISOR."/".$event->getString("friendly_url");
				
			} else {
				$sbmLink = EVENT_DEFAULT_URL."/results.php?id=".$event->getNumber("id");
                $sbmLinkShare = EVENT_DEFAULT_URL."/".ALIAS_SHARE_URL_DIVISOR."/".$event->getString("friendly_url");
			}
			$facebook    = "href=\"http://www.facebook.com/sharer.php?u=".$sbmLinkShare."&amp;t=".urlencode($event->getString("title"))."\" target=\"_blank\"";
	...
	if (SOCIAL_BOOKMARKING == "on") {
        ...
        if (THEME_SHARE_ITEMS) {
            $share_icon     = "<li><a rel=\"nofollow\" ".($event->getNumber("id") ? "id=\"link_social_".htmlspecialchars($event->getNumber("id")).$type."\"" : "")." href=\"javascript:void(0);\" ".($user ? "onclick=\"enableSocialBookMarking(".$event->getNumber("id").", '".$type."', '".DEFAULT_URL."');\"" : "")." $socialbookmarking_style>".system_showText(string_strtolower(LANG_LABEL_SHARE))."</a></li>";
        }
    }
	...
	if ($members) {
		if (($id == sess_getAccountIdFromSession()) || ($members != "profile")) {
			$links .= "<li><a rel=\"nofollow\" id=\"favoritesRemove_".$event->getNumber("id")."\" href=\"".$remove_favorites_link."\" ".$remove_favorites_click." ".$remove_favorites_style.">".system_showText(LANG_ICONQUICKLIST_REMOVE)."</a></li>";
		}
	}else {
		$links .= "<li><a rel=\"nofollow\" ".($event->getNumber("id") ? "id=\"favorites_".$event->getNumber("id")."\"" : "")." href=\"".$include_favorites_link."\" class=\"".$includes_favorites_class."\" ".$include_favorites_click." ".$include_favorites_style.">".(THEME_FAVORITES_ICON ? $favorites_imgE : system_showText(LANG_ICONQUICKLIST_ADD))."</a></li>";
	}
	if (THEME_DETAIL_PRINT && ($level->getDetail($icon_event_level)) == "y" && ($typePreview == "detail")) {
