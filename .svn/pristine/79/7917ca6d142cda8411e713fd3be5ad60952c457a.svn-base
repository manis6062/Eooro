<?
	 ...
	 * @access Public
	 */
        include_once EDIRECTORY_ROOT.'/extras/core/library/validator.php';
	class LocationCounter extends Handle {
        ...
        function getLastLevelLocationCounter($locationLevel, $module, $orderField, $limit) {
            ...
            if ($locationLevel) {
                ...
                $location_1_Id = $validator->escape( $_COOKIE['location_geoip_id'] );
                
                $sql = "SELECT count, title, full_friendly_url, location_id FROM ".ucfirst($module)."_LocationCounter WHERE location_level = ".$locationLevel." AND title != '' AND location_1_id = $location_1_Id ORDER BY ".$orderField." LIMIT ".$limit;
                $result = $dbObj->query($sql);
                if (mysql_num_rows($result)) {
        ...
        function getLocationIDByLocationLevel($location_level) {
            $dbMain = db_getDBObject(DEFAULT_DB, true);
