<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /conf/contractors_scheme.inc.php
	# ----------------------------------------------------------------------------------------------------
	...
	//set default scheme
	$edir_default_scheme = "contractors";

	//set all available schemes separated by comma
	$edir_schemes = "contractors";
	$edir_schemenames = "Contractors";

	//code to setup one specific scheme from all available schemes
	$edir_scheme = "contractors";

	if (DEMO_LIVE_MODE == 0) {
		@include_once(EDIRECTORY_ROOT.'/custom/domain_'.SELECTED_DOMAIN_ID.'/theme/contractors_scheme.inc.php');
	} else {
