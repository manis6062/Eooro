<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /controller/event/rewrite.php
	# ----------------------------------------------------------------------------------------------------
    ...
    if ($_GET["url_full"] && (string_strpos($_GET["url_full"], ALIAS_EVENT_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false || string_strpos($_GET["url_full"], ALIAS_EVENT_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false)) {

		$url = string_replace_once(EDIRECTORY_FOLDER."/".ALIAS_EVENT_MODULE."/", "", $_GET["url_full"]);
		$parts = explode("/", $url);
        
		if (string_strpos($_GET["url_full"], ALIAS_EVENT_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false ){
			$browsebycategory = true;
			
			for ($i = 1; $i<count($parts); $i++){
					$_GET["category".$i] = $parts[$i];
			}
		} else if (string_strpos($_GET["url_full"], ALIAS_EVENT_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false){
			...
			if (!$parts[1] && !$parts[3]) {
				header("Location: ".EVENT_DEFAULT_URL."/results.php");
				exit;
			}
	...
	##################################################
	if ($_GET["category1"]) {
		$sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber("0")." AND friendly_url = ".db_formatString($_GET["category1"])." AND enabled = 'y' LIMIT 1";
	...
	if ($_GET["category2"] && $_GET["category_id"] && !$failure) { 
		$sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND friendly_url = ".db_formatString($_GET["category2"])." AND enabled = 'y' LIMIT 1";
		$result = $dbObj->query($sql);
		$aux = mysql_fetch_assoc($result);
		$_GET["category_id"] = $aux["id"];
		if (!$_GET["category_id"]) $failure = true;
    ...
    if ($_GET["category3"] && $_GET["category_id"] && !$failure) { 
        $sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND friendly_url = ".db_formatString($_GET["category3"])." AND enabled = 'y' LIMIT 1";
        $result = $dbObj->query($sql);
        $aux = mysql_fetch_assoc($result);
        $_GET["category_id"] = $aux["id"];
        if (!$_GET["category_id"]) $failure = true;
    ...
    if ($_GET["category4"] && $_GET["category_id"] && !$failure) {
        $sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND friendly_url = ".db_formatString($_GET["category4"])." AND enabled = 'y' LIMIT 1";
        $result = $dbObj->query($sql);
        $aux = mysql_fetch_assoc($result);
        $_GET["category_id"] = $aux["id"];
        if (!$_GET["category_id"]) $failure = true;
    ...
    if ($_GET["category5"] && $_GET["category_id"] && !$failure) {
        $sql = "SELECT * FROM EventCategory WHERE category_id = ".db_formatNumber($_GET["category_id"])." AND friendly_url = ".db_formatString($_GET["category5"])." AND enabled = 'y' LIMIT 1";
        $result = $dbObj->query($sql);
        $aux = mysql_fetch_assoc($result);
        $_GET["category_id"] = $aux["id"];
        if (!$_GET["category_id"]) $failure = true;
