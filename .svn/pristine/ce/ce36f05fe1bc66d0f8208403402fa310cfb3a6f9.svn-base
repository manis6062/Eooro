...
(function($) {
	...
	$.csv2table={
		...
		loadImg  : (new Image()).src=DEFAULT_URL+'/images/img_loading.gif',  //Default loading IMG
//		sortNImg : (new Image()).src='./img/icon-n.gif',         //Default sort IMG N
//		sortDImg : (new Image()).src='./img/icon-d-green.gif',   //Default sort IMG D
//		sortAImg : (new Image()).src='./img/icon-a-green.gif',   //Default sort IMG A
		setting  : [],
		data     : [],
		_rowsAry : [],
		_doc     : document,
	...
	$.fn.csv2table= function (url,setting){ 
		...
		function  mkTable(id,rowsAry){
			...
			for (var k=0; k<col; k++) {
				...
				htm+= "<th id='"+id+"-th-"+k+"'><span class='import-span-nowrap'>"
			...
			for (var i=s; i<row; i++) {
					...
					//列の処理
					for (var j=0; j<col; j++) {
						htm+= "<td><span class='import-span-nowrap'>"
						   + rowsAry[i][j]
						   + "</span></td>";
					}
					htm+= "</tr>";
		...
		function escapeStrComma(col_sep,row_sep,oj,removeDoubleQuote){
			...
			col_sep2 = col_sep;
			if (col_sep == '*' || col_sep == '|' || col_sep == '\\' || col_sep == '/' || col_sep == '.') var col_sep2 = '\\' + col_sep;

			var reg='(["](.|(\r\n))*?(["]$|["][' + (col_sep == '\\'? '\\' + col_sep: col_sep) + '('+op.row_sep_reg+')]))',
			...
			escape= escape.replace(
				...
				function (after,before,index) {
					after= after
							.replace(/(\r\n)(?!$)/g,dmystr_rn)
							.replace(new RegExp(col_sep2 + "(?!$)", "g"),dmystr_comma)
					return after
			...
			for(var i=0;i<rowlen;i++){
				...
				for(var j=0;j<collen;j++){
					try{
						b[i][j]=$.trim(r[i][j])
							.replace(/^"|"$/g,rdq)
							.replace(new RegExp(dmystr_comma,"g"),col_sep)
