<?
	...
	class Domain extends Handle {
		 ...
		 * @access Private
		 */
		const CUSTOM_FOLDERS = "conf,content_files,editor_files,extra_files,image_files,export_files,import_files,lang,location,payment,sitemap,socialnetwork,theme,cache_full,cache_partial,cacheExpirationQueries,cacheUpdateToken,cacheVerbose,cache_filter";
			...
			if (mysql_num_rows($result) > 0) {
				...
				if ($sample_link) {
					...
					if ($new_link) {
						...
						// Insert Dashboard
						$sql = "INSERT INTO `Dashboard` (`domain_id`) VALUES ($this->id);";
						$dbObj->Query($sql);
		...
		function createCustomSSLFile () {
			...
			if ($fileSSL = fopen($fileSSLPath, "w+")) {
				...
				$buffer .= "# SSL".PHP_EOL;
				$buffer .= "# ----------------------------------------------------------------------------------------------------".PHP_EOL;
				$buffer .= "if (SSL_ENABLED == \"on\" && (string_strpos(\$_SERVER[\"PHP_SELF\"], \"iapp\") === false)) {".PHP_EOL;
