...
//					setting_get("sitemgr_email", $sitemgr_email_message);
			...
			if ($section == "sitemgr") {
				...
				if (!$emailNotificationObj = system_checkEmail(SYSTEM_FORGOTTEN_PASS)) {
					...
					$error = false;
					$return  = system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_FORGOTTEN_PASS);
					##################################################
