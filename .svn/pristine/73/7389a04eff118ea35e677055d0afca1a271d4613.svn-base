<?
	...
	if (sess_getSMIdFromSession() || $_GET["id"] == SELECTED_DOMAIN_ID) header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/domain");

	$url_redirect = "".DEFAULT_URL."/".SITEMGR_ALIAS."/domain";
	...
	if ($id) {
        ...
        $message = 2;
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/domain/index.php?message=".$message."&screen=$screen&letter=$letter");
		exit;
	}
	...
	if (DEMO_LIVE_MODE) { ?>
		...
		<script language="javascrip" type="text/javascript">
            $(document).ready(function(){
                ...
                setTimeout("window.location=\"<?=DEFAULT_URL;?>/<?=SITEMGR_ALIAS?>/domain/\"", 5000);
            });
		</script>
		...
		<div class="default-margin">
			...
			<form name="domain" method="post" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>">
				...
				<p class="informationMessage">
					...
					<br />
					<?=system_showText(LANG_SITEMGR_DOMAIN_DELETEQUESTION2)?> <a href="<?=DEFAULT_URL;?>/<?=SITEMGR_ALIAS?>/faq/faq.php?keyword=<?=urlencode("delete a site");?>" target="_blank"><?=system_showText(LANG_SITEMGR_DOMAIN_DELETEQUESTION3)?></a>
				</p>
			...
			<form id="formdomaindeletecancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/domain/index.php" method="post">
				<input type="hidden" name="letter" value="<?=$letter?>" />
				<input type="hidden" name="screen" value="<?=$screen?>" />
			</form>
		</div>
