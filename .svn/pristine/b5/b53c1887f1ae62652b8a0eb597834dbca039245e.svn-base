<?
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST") && (!DEMO_LIVE_MODE)) {
		/* setting copyright */
		if (!customtext_set("footer_copyright", $copyright)) {
			if (!customtext_new("footer_copyright", $copyright)) {
				$error = true;
				$actions = "";
			}
		}
         ...
         * Twitter Feed
         */
        if(!setting_set("twitter_feed", $twitter_feed)) {
			if(!setting_new("twitter_feed", $twitter_feed)) {
				$error = true;
			}
		}        
                
        if(!setting_set("setting_googleplus_link", $setting_googleplus_link)) {
                if(!setting_new("setting_googleplus_link", $setting_googleplus_link)) {
                        $error = true;
                }
        }
	...
	*/
	...
	setting_get("setting_facebook_link", $setting_facebook_link);
        ...
        setting_get("twitter_feed", $twitter_feed);
