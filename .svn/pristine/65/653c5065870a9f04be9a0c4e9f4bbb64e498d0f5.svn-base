...
class breadcrumb {
	...
	function breadcrumb() {
		...
		// Is this a personal site?
		if (string_substr($_SERVER['REQUEST_URI'], 1, 1) == '~') {
	...
	function multiLanguage($string) {
		...
		} elseif ($string == ALIAS_BESTOF_URL_DIVISOR) {
			$string = system_showText(LANG_MENU_BESTOF);
		}
		return $string;
	}
	...
	function eDirStructure($item_id, $identifier, $type) {
		...
		if (!strcmp($type, LISTING_FEATURE_FOLDER)) {
			$this->module = ALIAS_LISTING_MODULE;
		} elseif (!strcmp($type, EVENT_FEATURE_FOLDER)) {
			$this->module = ALIAS_EVENT_MODULE;
		} elseif (!strcmp($type, ARTICLE_FEATURE_FOLDER)) {
			$this->module = ALIAS_ARTICLE_MODULE;
		} elseif (!strcmp($type, CLASSIFIED_FEATURE_FOLDER)) {
			$this->module = ALIAS_CLASSIFIED_MODULE;
		} elseif (!strcmp($type, PROMOTION_FEATURE_FOLDER)) {
			$this->module = ALIAS_PROMOTION_MODULE;
		} elseif (!strcmp($type, BLOG_FEATURE_FOLDER)){
			$this->module = ALIAS_BLOG_MODULE;
		} elseif ($type == "bestof") {
            $this->module = ALIAS_BESTOF_URL_DIVISOR;
		}
		...
		switch ($identifier) {
			case 'category':
				if ($this->module == ALIAS_LISTING_MODULE && USE_DOT_PHP_ON_ALLCATEGORIES_LINK == "off") {
                    $label = ucfirst(system_showText(LANG_MENU_BYCUISINE));
                    $byCuisine = true;
                } elseif ($this->module == ALIAS_BESTOF_URL_DIVISOR) {
                    $label = "bestof";
                    $bestof = true;
                } else {
		...
		if ($identifier == "category") {
			...
			while ($cat_id) {
				...
				$categoryChain[$i]['title'] = $categoryObj->getString("title");
				$categoryChain[$i]['friendly'] = $categoryObj->getString("friendly_url");
				$i++;
				$cat_id = $categoryObj->category_id;
			}
		...
		if ($label) {
			...
			if (EDIRECTORY_FOLDER != '') {
				...
				if (!strcmp($type, LISTING_FEATURE_FOLDER)) {
					$this->scriptArray[1] = ALIAS_LISTING_MODULE;
				} elseif (!strcmp($type, ARTICLE_FEATURE_FOLDER)) {
					$this->scriptArray[1] = ALIAS_ARTICLE_MODULE;
				} elseif (!strcmp($type, EVENT_FEATURE_FOLDER)) {
					$this->scriptArray[1] = ALIAS_EVENT_MODULE;
				} elseif (!strcmp($type, CLASSIFIED_FEATURE_FOLDER)) {
					$this->scriptArray[1] = ALIAS_CLASSIFIED_MODULE;
				} elseif (!strcmp($type, PROMOTION_FEATURE_FOLDER)) {
					$this->scriptArray[1] = ALIAS_PROMOTION_MODULE;
				} elseif (!strcmp($type, BLOG_FEATURE_FOLDER)){
					$this->scriptArray[1] = ALIAS_BLOG_MODULE;
				} elseif ($type == "bestof"){
					$this->scriptArray[1] = ALIAS_BESTOF_URL_DIVISOR;
				}
			} else {

				if (!strcmp($type, ALIAS_LISTING_MODULE)) {
					$this->scriptArray[1] = ALIAS_LISTING_MODULE;
				} elseif (!strcmp($type, ALIAS_ARTICLE_MODULE)) {
					$this->scriptArray[1] = ALIAS_ARTICLE_MODULE;
				} elseif (!strcmp($type, ALIAS_EVENT_MODULE)) {
					$this->scriptArray[1] = ALIAS_EVENT_MODULE;
				} elseif (!strcmp($type, ALIAS_CLASSIFIED_MODULE)) {
					$this->scriptArray[1] = ALIAS_CLASSIFIED_MODULE;
				} elseif (!strcmp($type, ALIAS_PROMOTION_MODULE)) {
					$this->scriptArray[1] = ALIAS_PROMOTION_MODULE;
				} elseif (!strcmp($type,ALIAS_BLOG_MODULE)){
					$this->scriptArray[1] = ALIAS_BLOG_MODULE;
				}
			}
            ...
            $keyPage = array_search('page', $this->scriptArray);
            $numDirs = count($this->scriptArray);
            if ($keyPage) {
                if ($numDirs > $keyPage) {
                    for ($i = $keyPage; $i <= $numDirs; $i++) {
                        unset($this->scriptArray[$i]);
                    }
                }
            }
            
            $keyLetter = array_search('letter', $this->scriptArray);
            if ($keyLetter) {
                if ($numDirs > $keyLetter) {
                    for ($i = $keyLetter; $i <= $numDirs; $i++) {
                        unset($this->scriptArray[$i]);
                    }
                }
            }
            ...
            //fix reviews
            if (string_strpos($_SERVER['REQUEST_URI'], ALIAS_REVIEW_URL_DIVISOR."/") !== false) {
                $keyPage = array_search(ALIAS_REVIEW_URL_DIVISOR, $this->scriptArray);
                if ($keyPage) {
                    if ($numDirs > $keyPage) {
                        for ($i = $keyPage; $i <= $numDirs; $i++)
                            unset($this->scriptArray[$i]);
            ...
            if (string_strpos($_SERVER['REQUEST_URI'], ALIAS_CHECKIN_URL_DIVISOR."/") !== false) {
                $keyPage = array_search(ALIAS_CHECKIN_URL_DIVISOR, $this->scriptArray);
                if ($keyPage) {
                    if ($numDirs > $keyPage) {
                        for ($i = $keyPage; $i <= $numDirs; $i++)
                            unset($this->scriptArray[$i]);
            ...
            //fix blog archive
            if (string_strpos($_SERVER['REQUEST_URI'], ALIAS_ARCHIVE_URL_DIVISOR."/") !== false) {
                $keyPage = array_search(ALIAS_ARCHIVE_URL_DIVISOR, $this->scriptArray);
                if ($keyPage) {
                    if ($numDirs > $keyPage) {
                        for ($i = $keyPage; $i <= $numDirs; $i++)
                            unset($this->scriptArray[$i]);
			...
			for ($i = 0; $i < $numDirs; $i++) {
				...
				switch ($current_dir) {
                    case ucfirst(system_showText(LANG_CATEGORIES_SUBCATEGS)):
						$dir = $previous_dir . $this->alias_allcategories.(USE_DOT_PHP_ON_ALLCATEGORIES_LINK == "on" ? ".php" : "");
						$break_dir = true;
						break;
					case ucfirst(system_showText(LANG_MENU_BYCUISINE)):
						$dir = $previous_dir . $this->alias_allcategories.(USE_DOT_PHP_ON_ALLCATEGORIES_LINK == "on" ? ".php" : "");
						$break_dir = true;
						break;
					case ucfirst(system_showText(LANG_LOCATIONS)):
						$dir = $previous_dir . ALIAS_ALLLOCATIONS_URL_DIVISOR.".php";
						$break_dir = true;
						break;
				}
	...
	function show_auxbreadcrumb(){
        ...
        if (string_strpos($_SERVER["REQUEST_URI"], ALIAS_ADVERTISE_URL_DIVISOR.".php") !== false) {
			$auxbreadcrumb .= "<a href=\"".DEFAULT_URL."\" title=\"".system_showText(LANG_MENU_HOME)."\">".system_showText(LANG_MENU_HOME)."</a><span class=\"split\">&nbsp;&nbsp;".THEME_BREADCRUMB_SEP."&nbsp;&nbsp;</span>";
			$auxbreadcrumb .= system_showText(LANG_MENU_ADVERTISE);
		}
        
		if (string_strpos($_SERVER["REQUEST_URI"], ALIAS_CONTACTUS_URL_DIVISOR.".php") !== false) {
			$auxbreadcrumb .= "<a href=\"".DEFAULT_URL."\" title=\"".system_showText(LANG_MENU_HOME)."\">".system_showText(LANG_MENU_HOME)."</a><span class=\"split\">&nbsp;&nbsp;".THEME_BREADCRUMB_SEP."&nbsp;&nbsp;</span>";
		...
		if (string_strpos($_SERVER["REQUEST_URI"], ALIAS_SITEMAP_URL_DIVISOR.".php") !== false) {
			$auxbreadcrumb .= "<a href=\"".DEFAULT_URL."\" title=\"".system_showText(LANG_MENU_HOME)."\">".system_showText(LANG_MENU_HOME)."</a><span class=\"split\">&nbsp;&nbsp;".THEME_BREADCRUMB_SEP."&nbsp;&nbsp;</span>";
			$auxbreadcrumb .= system_showText(LANG_MENU_SITEMAP);
		}
        
        if (string_strpos($_SERVER["REQUEST_URI"], ALIAS_BESTOF_URL_DIVISOR) !== false) {
			$auxbreadcrumb .= "<a href=\"".DEFAULT_URL."\" title=\"".system_showText(LANG_MENU_HOME)."\">".system_showText(LANG_MENU_HOME)."</a><span class=\"split\">&nbsp;&nbsp;".THEME_BREADCRUMB_SEP."&nbsp;&nbsp;</span>";
			$auxbreadcrumb .= system_showText(LANG_MENU_BESTOF);
		}
