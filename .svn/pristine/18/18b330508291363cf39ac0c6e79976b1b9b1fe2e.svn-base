<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /conf/diningguide_scheme.inc.php
	# ----------------------------------------------------------------------------------------------------
	...
	//set default scheme
	$edir_default_scheme = "diningguide";

	//set all available schemes separated by comma
	$edir_schemes = "diningguide";
	$edir_schemenames = "Dining Guide";

	//code to setup one specific scheme from all available schemes
	$edir_scheme = "diningguide";

	if (DEMO_LIVE_MODE == 0) {
		@include_once(EDIRECTORY_ROOT.'/custom/domain_'.SELECTED_DOMAIN_ID.'/theme/diningguide_scheme.inc.php');
	} else {
