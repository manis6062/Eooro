<?
	...
	} elseif ($featuredBlogCategory) {
		$title = "blog";
		$table = "BlogCategory";
		$categoryObj = new BlogCategory($category_id);
		$categoryNoRecord = LANG_SITEMGR_BLOG_CATEGORY_NORECORD;
	}
    ...
    if ($categories) {
        ...
        if ($path_count > FEATUREDCATEGORY_LEVEL_AMOUNT) { ?>
            <script type="text/javascript">
                $(location).attr('href', '<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=$title?>categs/');
            </script>
        <?
            exit;
        }
        ...
        <!--
            ...
            function switchCheck(link, obj) {
                ...
                if(link == true){
                    if (open==true){
                        $('.table-itemlist input').attr('checked', '');
                        ...
                        $('.table-itemlist input').attr('checked', 'checked');
        ...
        <form name="fm_featuredCategories" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
            ...
            <table  class="table-itemlist">
                <thead>
                    <tr>
                            ...
                            <? if ($path_count < FEATUREDCATEGORY_LEVEL_AMOUNT && !$hideSubCateg) { ?>
                            <th><?=system_showText(LANG_SITEMGR_SUBCATEGORIES)?></th>
                            <th><?=system_showText(LANG_SITEMGR_FEATUREDSUBCATEGORY_PLURAL)?></th>
                        <? } ?>
                ...
                foreach ($categories as $category) {
                    ...
                    <tr>
                        ...
                        <td nowrap="nowrap" class="main-options">
                            <? if ($path_count < FEATUREDCATEGORY_LEVEL_AMOUNT) { ?>
                                <a href="<?=$url_redirect?>?category_id=<?=$id?>">
            ...
            <div align="left">
                <button type="submit" name="bt_submit" class="input-button-form" value="<?=system_showText(LANG_SITEMGR_SAVE)?>"><?=system_showText(LANG_SITEMGR_SAVE)?></button>
            </div>
        </form>
