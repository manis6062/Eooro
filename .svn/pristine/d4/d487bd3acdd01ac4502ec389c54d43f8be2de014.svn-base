<?
	...
	if ($id) {
		...
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/smaccount/");
		exit;
	}
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		$smaccount->delete();
        $message = 4;
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/smaccount/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
		exit;
	}
...
<div id="main-right">
	...
	<div id="content-content">
		<div class="default-margin">
			...
			<form id="formsmaccountdeletecancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/smaccount/<?=(($search_page) ? "search.php" : "index.php");?>" method="post">
