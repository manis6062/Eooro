<?
	...
	function image_uploadForItem($tmp_name, $prefix, $fullwidth, $fullheight, $thumbwidth="", $thumbheight="", $force_main = false) {
		...
		if ( ($types[$info[2]] == "JPG") || ($types[$info[2]] == "GIF") || ($types[$info[2]] == "PNG") ) {
			...
			$dir = DEFAULT_URL."/custom/domain_".SELECTED_DOMAIN_ID."/image_files";
			$dir_location = EDIRECTORY_ROOT. "/custom/domain_".SELECTED_DOMAIN_ID."/image_files";

			//Extract Image URL
			$imagename = $dir . "/" . $imageObj->prefix . 'photo_' . $imageObj->id. "." . strtolower($imageObj->type);
			$image_location = $dir_location . "/" . $imageObj->prefix . 'photo_' . $imageObj->id. "." . strtolower($imageObj->type);
			
			// Extract Image Mime
			$info = getimagesize($imagename);
			$type = $info['mime'];

				compress_image($type, $imagename, $image_location);
			...
			if ($thumbwidth && $thumbwidth) {
				...
				//Extract Thumb URL
				$thumbname = $dir . "/" . $thumbObj->prefix . 'photo_' . $thumbObj->id. "." . strtolower($thumbObj->type);
				$thumb_location = $dir_location . "/" . $thumbObj->prefix . 'photo_' . $thumbObj->id. "." . strtolower($thumbObj->type);
				
				// Extract Image Mime
				$info = getimagesize($thumbname);
				$type = $info['mime'];

				compress_image($type, $thumbname, $thumb_location);
...
/**
 * @Compress Image
 * @Parameters Image URL, Destination URL, Quality
 * Type: PNG from 0 (no compression) to 9. 
 * Type JPG quality (1 - 100), 1 is worst quality, super comrpessed
 */
	function compress_image($type, $image_url, $image_location) {
		...
		if($type == "image/jpeg"){
			...
			imagejpeg($image, $image_location, $compression_level_jpg);


		} elseif ($type == "image/gif") {

			//GIF images could not be compressed, convert it to JPG and compress it
			//And save as JPG
			...
			imagejpeg($image, $image_location, $compression_level_gif);
			...
			imagepng($image, $image_location, $compression_level_png);
		}
