<?
    ...
    if($reviewObj->getString('item_type') == 'listing') {
        ...
        if($emailNotificationObj = system_checkEmail(SYSTEM_APPROVE_REVIEW)) {
            ...
            system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", $itemObj->id, $contactObj->account_id, SYSTEM_APPROVE_REVIEW);
        }
    }
    /* */
