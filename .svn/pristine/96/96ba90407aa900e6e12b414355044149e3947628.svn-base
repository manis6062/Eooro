<?php
     ...
     * @method getPromotionByFriendlyURL
	 * @method setListingId
	...
	class Promotion extends Handle {
		...
		function Save() {
			...
			if ($this->id) {
                ...
                activity_updateRecord(SELECTED_DOMAIN_ID, $this->id, $this->name, "item", "promotion");
        ...
        function getTagLine($link = false){
            if (!$link){
                $link = DEFAULT_URL."/".ALIAS_PROMOTION_MODULE."/".$this->getString("friendly_url");
            }
            ...
            if ($this->id > 0 && $this->listing_status == 'A') {
                ...
                foreach ($this->data_in_array as $key => $value) {
                    ...
                    if ($key == "listing_id" && $value > 0) {
                        ...
                        $add_info[(API_IN_USE == "api3" ? "remaining" : "total_amount")] = (int)$this->amount;
                 ...
                 */
                $add_info["friendly_url"] = PROMOTION_DEFAULT_URL."/".ALIAS_SHARE_URL_DIVISOR."/".$this->friendly_url;
        ...
        function GetInfoToApp($array_get, &$aux_returnArray, &$aux_fields, &$items, &$auxTable, &$aux_Where) {
            ...
            if ($searchBy) {
                if ($searchBy == "keyword" || $searchBy == "keyword_where") {
                    ...
                    if (API_IN_USE == "api3") {
                        $searchReturn["select_columns"] = str_replace("name,", "name as title,", $searchReturn["select_columns"]);
                    }
