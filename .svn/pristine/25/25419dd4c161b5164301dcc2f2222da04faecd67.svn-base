<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /edir_core/deal/detail_info.php
	# ----------------------------------------------------------------------------------------------------

    if (EXTRA_FIELDS_SIDEBAR && !$promotionMsg && !$hideDetail) { ?>
                ...
                <div class="detail-info">
                    <div class="info">
                        <h1 class="counter-title">
                            <img src="<?=THEMEFILE_URL?>/<?=EDIR_THEME?>/images/iconography/icon-time.png" alt="<?=system_showText(LANG_LABEL_TIMELEFT)?>" /> <?=system_showText(LANG_LABEL_TIMELEFT)?>
                        </h1>
                        <ul class="counter">
                            <li id="timeLeft">
                                <? if (!$user) {  ?>
                        ...
                        <div class="action">
                            ...
                            <? } else { ?>
                                <p><strong><?=system_showText(DEAL_VALUE)?>:</strong> <?=$deal_real_value;?></p>
                                <p><strong><?=system_showText(LANG_PROMOTION_FEATURE_NAME_PLURAL)." ".system_showText(DEAL_LEFTAMOUNT)?>:</strong> <label id="updateDealsLeft"><?=$deal_left;?></label></p>
