<?
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if ($hiddenValue) {
            ...
            $mailListObj->Delete();
            header("Location: $url_redirect/arcamailerexport.php?message=1&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "" )."#info");
            exit;
        } else {
            ...
            if (validate_form("mailapp", $_POST, $message_mailapp)) {
                ...
                header("Location: ".$url_redirect."/arcamailerexport.php?message=".$message."&screen=".$screen."&letter=".$letter."#info");
                exit;
        ...
        if ($mailListObj->getString("status") == "F" && file_exists($filePath)) {
            ...
            header("Location: $url_redirect/arcamailerexport.php?emessage=2&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "" )."#info");
            exit;
        }
    }
