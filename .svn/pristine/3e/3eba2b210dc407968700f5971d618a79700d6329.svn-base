<?
	...
	$categoryDropDown = html_selectBox("search_category_id", $nameArray, $valueArray, $search_category_id, "", "class='input-dd-form-listing'", system_showText(LANG_LABEL_SELECT_ALLCATEGORIES));
	...
	************************************************/
    ...
    if ($search_title) {
        ...
        $search_for_keyword_fields[] = "Listing_Summary.fulltextsearch_what";
	...
	$paging_url = DEFAULT_URL."/".SITEMGR_ALIAS."/".LISTING_FEATURE_FOLDER."/search.php";
	
	$url_search_params = system_getURLSearchParams((($_POST)?($_POST):($_GET)));
    $manageOrder = system_getManageOrderBy($_POST["order_by"] ? $_POST["order_by"] : $_GET["order_by"], "Listing_Summary", LISTING_SCALABILITY_OPTIMIZATION, $fieldsManage, true);
	...
	$pageObj = new pageBrowsing("Listing_Summary", $screen, RESULTS_PER_PAGE, $manageOrder, "title", $letter, $where, $fieldsManage);
