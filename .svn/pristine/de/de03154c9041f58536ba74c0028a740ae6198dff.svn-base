...
//        }
		...
		if (empty($error)) {
            ...
            if ($receiver == "friend") {
                $error = false;
                $return = system_mail($to, $subject, $message, $from, $emailNotificationObj->content_type, '', '', $error, '', '', $from, "", $obj->account_id, SYSTEM_EMAIL_TOFRIEND);
            } else {
                if ($emailNotificationObj = system_checkEmail(SYSTEM_NEW_LEAD)) {
                    ...
                    $error = false;
                    $return = system_mail($to, $subject, $body, $from, $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, '', '', $from, "", $obj->account_id, SYSTEM_NEW_LEAD);
                } else {
                    $error = false;
                    $return = system_mail($to, $subject, $message, $from, $emailNotificationObj->content_type, '', '', $error, '', '', $from, "", $obj->account_id, SYSTEM_NEW_LEAD);
                }
            }
