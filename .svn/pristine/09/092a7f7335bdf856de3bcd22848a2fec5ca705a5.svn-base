<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/code/home_categories.php
	# ----------------------------------------------------------------------------------------------------
	...
	if (LISTINGCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
		$sql = "SELECT id, title, friendly_url, active_listing FROM ListingCategory WHERE category_id = '0' ".($listing_featuredcategory ?"AND featured = 'y'":"")." AND title <> '' AND friendly_url <> '' AND enabled = 'y' ORDER BY title, active_listing DESC LIMIT 20";
		$categories_listing = system_generateXML("categories", $sql, SELECTED_DOMAIN_ID);
	} else {
		$categories_listing = $catObj->retrieveAllCategoriesXML($listing_featuredcategory);
	}
	...
	if (EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on") {
		...
		if (EVENTCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			...
			$sql = "SELECT id, title, friendly_url, active_event FROM EventCategory WHERE category_id = '0' ".($event_featuredcategory?"AND featured = 'y'":"")." AND title <> '' AND friendly_url <> '' AND enabled = 'y' ORDER BY title, active_event DESC LIMIT 20";
			$result = $dbCatObj->query($sql);
			$categories_event = false;
			while ($row = mysql_fetch_assoc($result)) $categories_event[] = $row;
			unset($dbCatObj);
		} else {
			$categories_event = $catObj->retrieveAllCategories($event_featuredcategory);
		}
	}
	...
	if (CLASSIFIED_FEATURE == "on" && CUSTOM_CLASSIFIED_FEATURE == "on") {
		...
		if (CLASSIFIEDCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$dbCatObj = db_getDBObJect();
			$sql = "SELECT id, title, friendly_url, active_classified FROM ClassifiedCategory WHERE category_id = '0' ".($classified_featuredcategory?"AND featured = 'y'":"")." AND title <> '' AND friendly_url <> '' AND enabled = 'y' ORDER BY title, active_classified DESC LIMIT 20";
			$result = $dbCatObj->query($sql);
			$categories_classified = false;
			while ($row = mysql_fetch_assoc($result)) $categories_classified[] = $row;
			unset($dbCatObj);
		} else {
			$categories_classified = $catObj->retrieveAllCategories($classified_featuredcategory);
		}
	}
	...
	if (ARTICLE_FEATURE == "on" && CUSTOM_ARTICLE_FEATURE == "on") {
		...
		if (ARTICLECATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			$dbCatObj = db_getDBObJect();
			$sql = "SELECT id, title, friendly_url, active_article FROM ArticleCategory WHERE category_id = '0' ".($article_featuredcategory?"AND featured = 'y'":"")." AND title <> '' AND friendly_url <> '' AND enabled = 'y' ORDER BY title, active_article DESC LIMIT 20";
			$result = $dbCatObj->query($sql);
			$categories_article = false;
			while ($row = mysql_fetch_assoc($result)) $categories_article[] = $row;
			unset($dbCatObj);
		} else {
			$categories_article = $catObj->retrieveAllCategories($article_featuredcategory);
		}
	}
	...
	if (CUSTOM_HAS_PROMOTION == "on" && PROMOTION_FEATURE == "on" && CUSTOM_PROMOTION_FEATURE == "on"){
		...
		if (LISTINGCATEGORY_SCALABILITY_OPTIMIZATION == "on") {
			...
			$categories_deal = $catObj->retrieveAllCategoriesXML($listing_featuredcategory);
		}
	}
