	...
	extract($_GET);
		...
		*******************************************************************************/
		...
		if (!$second_step) {
                       ...
                       $itemid = is_numeric($_GET['itemid']) ? $_GET['itemid'] : 'null'; 
                       if($accountid && $itemid)
                       {
                           $account_id = sess_getAccountIdFromSession();
                           $id = $itemid;
                           DBQuery::execute(function() use ($domain,$account_id,$id,&$total_listing_sum) {
                                $sql = $domain->prepare("SELECT count(id) FROM Listing WHERE account_id = :account_id"
                                                                ." AND id=:id");
                                $parameters = array(
                            ...
                            unset($dbObject);
                            if ($total_listing_sum <= 100 && sess_getAccountIdFromSession()) {
                                    $listings = db_getFromDB("listing", "id", mysql_real_escape_string($itemid), "", "title", "array", SELECTED_DOMAIN_ID);
                            } else {
			...
			if ($listing_id) {
                ...
                include_once '../../classes/class_Listing.php';
		if ($listings) {
			$levelObj = new ListingLevel();

			$bill_info["no_subscription_bill"] = "0.00";

			foreach ($listings as $each_listing) {

				$each_listing['status'] == "P" ? $each_listing = (array) new ListingPending($each_listing['id']): null; // Pending Modifcation
				...
				$category_amount = 0;
                                DBQuery::execute(function() use ($domain,&$category_amount,$each_listing) {
                                    $sql = $domain->prepare("SELECT category_id FROM Listing_Category WHERE listing_id = :listing_id");
                                    $parameters = array(
                                        ':listing_id'=>$each_listing["id"]
                                    );
                                    $sql->execute($parameters);
                                    if($rows = $sql -> rowCount()){
				...
				$bill_info["listings"]["{$each_listing["id"]}"]["location_1"]      = $each_listing["location_1"];
				$bill_info["listings"]["{$each_listing["id"]}"]["title"]           = htmlentities($each_listing["title"]);
				...
				$bill_info["listings"]["{$each_listing["id"]}"]["address"]         = $each_listing["address"];
				$bill_info["listings"]["{$each_listing["id"]}"]["monthly"]         = $each_listing["custom_checkbox3"];
				$bill_info["listings"]["{$each_listing["id"]}"]["gracePeriodUsed"]         = $each_listing["custom_checkbox4"];
				$bill_info["listings"]["{$each_listing["id"]}"]["subscription"]    = $each_listing["custom_text2"];
				...
				//Subscribe need to check out code
				if (strpos($_COOKIE['showListing'], $each_listing["id"]) > -1) $bill_info["listings"]["{$each_listing["id"]}"]["needtocheckout"] = "y";

				!$each_listing["custom_text2"] ? $bill_info["listings"]["{$each_listing["id"]}"]["nonSubListing"] = "y" : null;
				unset($thisListing);
				...
				// Bill pricing, pass listing location 1
				$price  = CountryLoader::getPriceListing($each_listing["id"], $each_listing["location_1"]);
				$bill_info["listings"]["{$each_listing["id"]}"]["total_fee"]   = $price['price_listing'];
				$bill_info["listings"]["{$each_listing["id"]}"]["curr_symbol"] = $price['symbol'];
				...
				#---------------------------------------------------------
				# 		Bill Amount for listings with subscription
				#---------------------------------------------------------
				$listingObjN = new Listing($each_listing["id"]);			

				if($listingObjN->status == "P"):

					$listingObjN = new ListingPending($each_listing["id"]);

				endif;

				if($listingObjN->discount_id):

					$discountCodeObj = new DiscountCode($listingObjN->discount_id);

					if(($discountCodeObj->type =="duration") && ($discountCodeObj->status == "A") && $discountCodeObj->expire_date >= date('Y-m-d')):

						$noSubscriptionListings[] = $each_listing["id"];

						unset($_POST['listing_id'][$key]);

						$bill_info["no_subscription_bill"] += $bill_info["listings"]["{$each_listing["id"]}"]["total_fee"];
				...
				if($bill_info["listings"]["{$each_listing["id"]}"]["total_fee"])
					$bill_info["listings"]["{$each_listing["id"]}"]["total_fee"] = $bill_info["listings"]["{$each_listing["id"]}"]["total_fee"];
			...
			if (empty($bill_info["listings"])) unset($bill_info["listings"]);
                ...
                /**
                 * Modification for case Payment.
                 */
                if ( !$second_step ){
                    // todo : i need a global switch to enable / disable case
                    $owner_id=sess_getAccountIdFromSession();
                    $itemid = is_numeric($_GET['itemid']) ? $_GET['itemid'] : 'null'; 
                   
                   if($owner_id)
                   {

                       DBQuery::execute(function() use ($domain,$itemid,&$total_case_sum) {
                            $sql = $domain->prepare("SELECT count(case_id) FROM Opened_Cases WHERE owner_id = :owner_id"
                            ...
                            $parameters = array(
                                ...
                                if ($row = $sql->fetch(\PDO::FETCH_BOTH)) {
                                    $total_case_sum = $row[0];
                                }
                       });
                    unset($dbObject);
                    if ($total_case_sum <= 50) {
                        $key = array('owner_id', 'listing_id');
                        $val = array(sess_getAccountIdFromSession(), $itemid);
//                            $cases = db_getFromDB( 'case', 'owner_id', sess_getAccountIdFromSession(), '', 'case_id', 'array', SELECTED_DOMAIN_ID );
                            $cases = db_getFromDB( 'case', $key, $val, '', 'case_id', 'array', SELECTED_DOMAIN_ID );
                    } else {
                            $overcase_msg = system_showText(LANG_MSG_OVERITEM_MORETHAN)." ".$max_item_sum." ".system_showText(LANG_EVENT_FEATURE_NAME_PLURAL).". ".system_showText(LANG_MSG_OVERITEM_CONTACTADMIN).".";
                    }
                   }
                }
                else {
                    if ( $case_id ) {
                        $caseSize = count($case_id);
                        for( $i = 0; $i < $caseSize; $i++ ){
                            $by_key = array( 'case_id', 'owner_id' );
                            $by_value = array( db_formatNumber($case_id[$i]), sess_getAccountIdFromSession() );
                            $cases[] = db_getFromDB("case", $by_key, $by_value, "1", "case_id", "array", SELECTED_DOMAIN_ID);
                        }
                }
        }
                
                // if there are cases belonging to this user
                if ( $cases ) {
                    include_once EDIRECTORY_ROOT.'/classes/class_Opened_Cases.php';
                    // retrieving categories related with listing

                    $bill_info['case_total'] = "0";
                    foreach( $cases as $case ){
                        
                        DBQuery::execute(function() use ($domain,$case,&$rows) {
                            $sql        = $domain->prepare("SELECT review_title, reviewer_name FROM Review WHERE id=:id");
                            $parameters = array(
                                ':id'=>$case['review_id'],
                            );
                            $sql->execute($parameters);
                        ...
                        $caseObj = new Opened_Cases( $case['case_id'] );

                        // setting some of the bill information
                        $bill_info["cases"]["{$case["case_id"]}"]["title"]        = htmlentities($rows['review_title']);
                        $bill_info["cases"]["{$case["case_id"]}"]["member"]       = $rows['reviewer_name'];
                        $bill_info["cases"]["{$case["case_id"]}"]["level_number"] = '';
                        $bill_info["cases"]["{$case["case_id"]}"]["opened_date"]  = $case['opened_date'];

                        //Set discount price
                        $case_price = CountryLoader::getPriceCases($case['case_id'], $caseObj->getLocationCase($case['case_id']));
                        $bill_info["cases"]["{$case["case_id"]}"]["discount_id"] = $caseObj->discount_id;
                        $bill_info["cases"]["{$case["case_id"]}"]["total_fee"] 	 = $case_price['price_case'];

                        // Need to checkOut
                        $bill_info["cases"]["{$case["case_id"]}"]["needtocheckout"] = $caseObj->needToCheckOut() ? 'y' : 'n';
                        $bill_info["cases"]["{$case["case_id"]}"]["curr_symbol"] = $case_price['symbol'];
                        
                        // Setting total bill
                        //if($bill_info["cases"]["{$case["case_id"]}"]["total_fee"])
                        		$bill_info['case_total'] += $bill_info["cases"]["{$case["case_id"]}"]["total_fee"];
                                $bill_info["total_bill"] += $bill_info["cases"]["{$case["case_id"]}"]["total_fee"];
                    }         
                    // There is no case been payed.
                    if (empty($bill_info["cases"])) unset($bill_info["cases"]);
                }
		...
		*******************************************************************************/
		...
		if (!$second_step) {
			if (EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on") {
                DBQuery::execute(function() use ($domain,&$total_event_sum) {
					$sql = $domain->prepare("SELECT count(id) FROM Event WHERE account_id = :account_id");
					$parameters = array(
                                    ':account_id'=>sess_getAccountIdFromSession()
                                );
                	$sql->execute($parameters);
		...
		*******************************************************************************/
		if (!$second_step) { //last
			if (BANNER_FEATURE == "on" && CUSTOM_BANNER_FEATURE == "on") {  
             	DBQuery::execute(function() use ($domain,&$total_banner_sum) {
					$sql = $domain->prepare("SELECT count(id) FROM Banner WHERE account_id = :account_id");
					$parameters = array(
                                     ':account_id'=>sess_getAccountIdFromSession()
                               	 );
                    $sql->execute($parameters);
					if ($row = $sql->fetch(\PDO::FETCH_ASSOC)) {
						$total_banner_sum = $row[0];
                            ...
                            if ($total_banner_sum <= 50) { 
                                DBQuery::execute(function() use ($domain,&$banners) {
                                    $sqlBanner = "";
                                    $sqlBanner .= " SELECT * FROM Banner";
                                    $sqlBanner .= " WHERE account_id = :account_id ";
                                    $sqlBanner .= " AND ((unlimited_impressions = 'n' AND unpaid_impressions > '0') or (unlimited_impressions = 'y'))";
                                    $sqlBanner .= " ORDER BY caption";
                                    $sql = $domain->prepare($sqlBanner,array(PDO::MYSQL_ATTR_USE_BUFFERED_QUERY => false));
                                    $parameters = array(
                                        ':account_id'=>sess_getAccountIdFromSession()
                                    );
		...
		*******************************************************************************/
		...
		if (!$second_step) {
			if (CLASSIFIED_FEATURE == "on" && CUSTOM_CLASSIFIED_FEATURE == "on") {
                DBQuery::execute(function() use ($domain,&$total_classified_sum) {
					$sql = $domain->prepare("SELECT count(id) FROM Classified WHERE account_id = :account_id");
					$parameters = array(
                                    ':account_id'=>sess_getAccountIdFromSession(),
                                );
                                $sql->execute($parameters);
		...
		*******************************************************************************/
		...
		if (!$second_step) {
			if (ARTICLE_FEATURE == "on" && CUSTOM_ARTICLE_FEATURE == "on") {
                DBQuery::execute(function() use ($domain,&$total_article_sum) {
					$sql = $domain->prepare("SELECT count(id) FROM Article WHERE account_id = :account_id");
					$parameters = array(
                                    ':account_id'=>sess_getAccountIdFromSession()
                                );
                    $sql->execute($parameters);
		...
		*******************************************************************************/
		...
		$bill_info["total_bill"]   = sprintf('%0.2f', $bill_info["total_bill"]);
			...
			if (!($payment_method == "invoice") || ($bill_info["total_bill"] > 0)) {
				if (!($bill_info["listings"]) || ($bill_info["events"]) || ($bill_info["banners"]) || ($bill_info["classifieds"]) || ($bill_info["articles"]) || ($bill_info["custominvoices"])) {
					...
					if (!$bill_info["listings"]) foreach ($bill_info["listings"] as $id => $info) {
						...
						$category_amount = 0;
		                    DBQuery::execute(function() use ($domain,$listingObj,&$category_amount) {
		                        $sql = $domain->prepare("SELECT category_id FROM Listing_Category WHERE listing_id = :listing_id");
		                        $parameters = array(
		                            ':listing_id'=>$listingObj->getString("id")
		                        );
		                        $sql->execute($parameters);
