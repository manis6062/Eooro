	...
	class SettingSocialNetwork extends Handle {
		...
		function getSectionSettings($section = "") {
			if ($section) {
				...
				unset($dbMain);
				$sql = "SELECT name FROM Setting_Social_Network WHERE name LIKE '".$section."_%'".(!THEME_EMAIL_TOFRIEND ? " AND name NOT LIKE '%send_email_to_friend'" : "").(!THEME_DETAIL_PRINT ? " AND name NOT LIKE '%_print'" : "");
				$result = $dbObj->query($sql);
				unset($sett);
				while ($row = mysql_fetch_array($result)) {
