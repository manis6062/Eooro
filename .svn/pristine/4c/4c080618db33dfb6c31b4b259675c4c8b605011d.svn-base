<?
	...
	if ($aux_export_running["finished"] == "N" && LISTING_SCALABILITY_OPTIMIZATION == "on") {
        ...
        $exportFile = "export_Listing_".md5(uniqid(rand(), true)).".csv";
						...
						if (xmlhttp_checkexportprogress.status == 200) {
							...
							if (isNaN(current_progress)) {
								...
								if (parseInt(document.getElementById("export_progress").innerHTML) >= 100) {
									...
									document.getElementById("export_message").innerHTML = "<?=system_showText(LANG_SITEMGR_EXPORT_EXPORTDONE).(LISTING_SCALABILITY_OPTIMIZATION == "on" ? " <br /><a href=".DEFAULT_URL."/".SITEMGR_ALIAS."/export/exportfile.php?export_type=listing&filename=".$exportFile."&type=csv>".system_showText(LANG_SITEMGR_EXPORT_LASTFILE_MESSAGE)."</a>" : "");?>";
									document.getElementById("export_progress").innerHTML = "&nbsp;";
									document.getElementById("export_progress_percentage").innerHTML = "&nbsp;";
									removeExportControl();
								} else {
					...
					<p style="padding-top: 10px; text-align: center;">
						<span id="export_message">
							<a href="javascript:startExport();" class="standardLink" style="margin: 0; padding: 0; font-size: 15px;">
								...
								<? if (LISTING_SCALABILITY_OPTIMIZATION == "on") {
									if ($old_export_file) { ?>
                                        <a href="<?=DEFAULT_URL."/".SITEMGR_ALIAS."/export/exportfile.php?export_type=listing&filename=".$old_export_file."&type=csv"?>"><?=system_showText(LANG_SITEMGR_EXPORT_LASTFILE_MESSAGE)?></a>
									<? }
                                }
								?>
						</span>
