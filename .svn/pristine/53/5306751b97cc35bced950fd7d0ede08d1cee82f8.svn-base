	...
	if (TWOCHECKOUTPAYMENT_FEATURE == "on") {
		...
		if (!TWOCHECKOUT_LOGIN) {
			...
			<form name="twocheckoutform" target="_self" action="<?=TWOCHECKOUT_POST_URL?>" method="post">
				...
				<table align="center" width="95%" cellpadding="2" cellspacing="2" class="standard-table payment-twocheckout">
					<tr>
					...
					<tr>
						...
						<td><input type="tel" name="x_phone" value="<?=$twocheckout_x_phone?>" /></td>
					</tr>
					<tr>
						<th><?=system_showText(LANG_LABEL_EMAIL)?>:</th>
						<td><input type="email" name="x_email" value="<?=$twocheckout_x_email?>" /></td>
					</tr>
					<tr>
				...
				<? if ($payment_process == "signup") {
                    ...
                    $buttonGateway = "<button type=\"button\" id=\"twocheckoutbutton\" onclick=\"submitOrder();\">".system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))."</button>";
