<?
	...
	if ($_GET["url_full"] && (string_strpos($_GET["url_full"], ALIAS_CLASSIFIED_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false || string_strpos($_GET["url_full"], ALIAS_CLASSIFIED_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false)) {
		
		$url = string_replace_once(EDIRECTORY_FOLDER."/".ALIAS_CLASSIFIED_MODULE."/", "", $_GET["url_full"]);
		$parts = explode("/", $url);

		if (string_strpos($_GET["url_full"], ALIAS_CLASSIFIED_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false ){
			$browsebycategory = true;
			
			for ($i = 1; $i<count($parts); $i++){
					$_GET["category".$i] = $parts[$i];
			}
		} else if (string_strpos($_GET["url_full"], ALIAS_CLASSIFIED_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false){
