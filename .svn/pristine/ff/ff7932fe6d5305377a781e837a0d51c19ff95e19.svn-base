	...
	if (WORLDPAYPAYMENT_FEATURE == "on") {
		...
		if (!WORLDPAY_INSTID) {
			...
			<form name="worldpayform" target="_self" action="<?=WORLDPAY_HOST?>" method="post">
				...
				<table align="center" width="95%" cellpadding="2" cellspacing="2" class="standard-table payment-worldpay">
					<tr>
				...
				<? if ($payment_process == "signup") {
                    ...
                    $buttonGateway = "<button type=\"button\" id=\"worldpaybutton\" onclick=\"submitOrder();\">".system_highlightWords(system_showText(LANG_LABEL_PLACE_ORDER_CONTINUE))."</button>";
