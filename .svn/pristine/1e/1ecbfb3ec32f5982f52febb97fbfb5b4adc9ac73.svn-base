<?
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("blog", $_POST, $message_blog)) {
			...
			header("Location: $url_redirect/".(($search_page) ? "search.php" : "index.php")."?newest=".$newest."&message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : ""));
			exit;
		}
