<?
	...
	if (!$commenting_fb){
		header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".PROMOTION_FEATURE_FOLDER."/".(($search_page) ? "search.php" : "index.php")."?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
		exit;
	}
		...
		<div class="default-margin">
			...
			<div class="baseForm">
				...
				<form name="facebook_form" id="facebook_form" action="<?=system_getFormAction($_SERVER["PHP_SELF"])?>" method="post">
					...
					<?
					$detailLink = PROMOTION_DEFAULT_URL."/".ALIAS_SHARE_URL_DIVISOR."/".$promotion->getString("friendly_url");
				...
				<form id="formfacebookcancel" action="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=PROMOTION_FEATURE_FOLDER;?>/<?=(($search_page) ? "search.php" : "index.php");?>" method="get">
