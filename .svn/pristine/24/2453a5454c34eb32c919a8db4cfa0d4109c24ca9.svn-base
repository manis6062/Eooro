							...
							<td width="250" align="center">
								...
								if ($invoice_image) {
									...
									if ($imageObj->imageExists()) {
										$imgTag = $imageObj->getTag(true, IMAGE_INVOICE_LOGO_WIDTH, IMAGE_INVOICE_LOGO_HEIGHT, system_showText(LANG_LABEL_INVOICELOGO));
									}
								}
								if ($imgTag) {
									echo $imgTag;
								} else {
									echo "<img src=\"".DEFAULT_URL."/images/content/img_invoice.gif\" alt=\"".system_showText(LANG_LABEL_INVOICELOGO)."\" title=\"".system_showText(LANG_LABEL_INVOICELOGO)."\" border=\"0\" width=\"".IMAGE_INVOICE_LOGO_WIDTH."\" height=\"".IMAGE_INVOICE_LOGO_HEIGHT."\" />";
								}
								?>
							</td>
							<td width="150">
