	...
	function report_PrepareListingStatsReviewToEmail($reports, $item_id, $item_type, $default_url = DEFAULT_URL){
        ...
        if ($item_id) {
            ...
            */
            ...
            $sql = "SELECT * FROM Review WHERE item_type = '$item_type' AND item_id = ".htmlspecialchars($item_id)." AND review IS NOT NULL AND review != '' AND approved=1 AND added >= '$startDate' AND DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 30 DAY), '%Y%m%d') <= added AND added <= NOW() AND status = 'A'";
