	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("classified", $_POST, $message_classified) && is_valid_discount_code($_POST["discount_id"], "classified", $_POST["id"], $message_classified, $discount_error_num)) {
			...
			***************************************************************************************************/
			...
			if ($classified->getNumber("account_id") > 0) {
				if($message == 0) {
					...
					if($emailNotificationObj = system_checkEmail(SYSTEM_NEW_CLASSIFIED)) {
						...
						system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_NEW_CLASSIFIED);
					}
				}
			}
			if ($emailNotification) {
				if (!string_strpos($url_base, "/".SITEMGR_ALIAS."")) {
                    ...
                    if ($_POST["id"]) {
                        ...
                        $link_sitemgrmsg = "<a href=\"".$domain_url."/".SITEMGR_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/settings.php?id=".$classified->getNumber("id")."\" target=\"_blank\">".$domain_url."/".SITEMGR_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/settings.php?id=".$classified->getNumber("id")."</a><br /><br />";
                        $sentUp = 1;
                    } else {
                        ...
                        $link_sitemgrmsg = "<a href=\"".$domain_url."/".SITEMGR_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/view.php?id=".$classified->getNumber("id")."\" target=\"_blank\">".$domain_url."/".SITEMGR_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/view.php?id=".$classified->getNumber("id")."</a><br /><br />";
                        $sentNew = 1;
                    }
			...
			header("Location: $url_redirect/".(($search_page) ? "search.php" : "index.php")."?module=classified&process=".$process."&newest=".$newest."&message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "" )."");
			exit;
