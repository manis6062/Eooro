		...
		if ($validate_account && $validate_contact) {
			...
			******************************************************/
			...
			if ($emailNotificationObj = system_checkEmail(SYSTEM_NEW_PROFILE)) {
				...
				system_mail($contact->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contact->account_id, SYSTEM_NEW_PROFILE);
