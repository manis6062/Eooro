<?
    ...
    if ($id) {
        $check_id = db_getFromDB('post', 'id', $id);
        if (!$check_id->id)  {
            $error = system_showText(LANG_MSG_TOFRIEND3)." <a href=".DEFAULT_URL.">" .DEFAULT_URL." </a>".system_showText(LANG_MSG_TOFRIEND4)."<br />";
            $item_error = TRUE;
        }
	...
	if ($receiver == "owner") {
		...
		if (empty($subject)) {
			$subject = system_replaceEmailVariables($emailNotificationObj->subject, $obj->getNumber("id"), "post");
		...
		if (empty($error)) {
			...
			if ($receiver == "friend") {
				...
				if ($emailNotificationObj = system_checkEmail(SYSTEM_EMAIL_TOFRIEND)) {
					...
					$message .= system_replaceEmailVariables($emailNotificationObj->body, $obj->getNumber("id"), "post");
				}
