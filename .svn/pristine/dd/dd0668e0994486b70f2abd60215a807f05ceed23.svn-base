<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /sitemgr/blogcategs/featured.php
	# ----------------------------------------------------------------------------------------------------
	...
	$url_redirect = "".DEFAULT_URL."/".SITEMGR_ALIAS."/blogcategs/featured.php";
    $url_base = "".DEFAULT_URL."/".SITEMGR_ALIAS;
    $sitemgr = 1;
    $featuredBlogCategory = 1;
	...
	if (!isset($pop_categories)) system_setFreqActions('blog_featuredcateg', 'BLOG_FEATURE');
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['save'] == 1) {
		...
		if ($non_feat_categories != "") {
			system_changeFeaturedAtribute("BlogCategory", "$non_feat_categories", "n" );
        }
		if ($feat_categories != "") {
			system_changeFeaturedAtribute("BlogCategory", "$feat_categories", "y" );
        }
    ...
    $paging_url = DEFAULT_URL."/".SITEMGR_ALIAS."/blogcategs/featured.php";
...
<div id="main-right">
    <div id="top-content">
        <div id="header-content">
            <h1><?=string_ucwords(system_showText(LANG_SITEMGR_BLOG))?> - <?=string_ucwords(system_showText(LANG_SITEMGR_FEATUREDCATEGORY_PLURAL))?> </h1>
        </div>
    </div>
