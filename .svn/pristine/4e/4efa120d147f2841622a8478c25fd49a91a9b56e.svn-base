		...
		if (empty($error)) {
			...
			if ($receiver == "owner") {
                ...
                if ($emailNotificationObj = system_checkEmail(SYSTEM_NEW_LEAD)) {
                    ...
                    $error = false;
                    $return = system_mail($to, $subject, $body, $from, $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, '', '', $from, "", "", SYSTEM_NEW_LEAD);
                } else {
                    $error = false;
                    $return = system_mail($to, htmlspecialchars_decode($subject), $body, $from, 'text/plain', '', '', $error, '', '', $from, "", "", SYSTEM_NEW_LEAD);
                }
				...
				$error = false;
				$return = system_mail($to, $subject, $message, $from, $emailNotificationObj->content_type, '', '', $error, '', '', $from, "", "", SYSTEM_EMAIL_TOFRIEND);
