<?
    ...
    */
    ...
    if ($_GET["url_full"] && (string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false || string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false || string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_REVIEW_URL_DIVISOR) !== false || string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_CHECKIN_URL_DIVISOR) !== false)) {

        $url = string_replace_once(EDIRECTORY_FOLDER."/".ALIAS_LISTING_MODULE."/", "", $_GET["url_full"]);

        $parts = explode("/", $url);

        if (string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR) !== false) {
        ...
        } elseif (string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_LOCATION_URL_DIVISOR) !== false) {
            $browsebylocation = true;

            for ($i = 1; $i < count($parts); $i++) {
                $_GET["friendLoc" . $i] = $parts[$i];
            }
        } elseif (string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_REVIEW_URL_DIVISOR) !== false) {
            $browsebyitem = true;
        ...
        } elseif (string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_CHECKIN_URL_DIVISOR) !== false) {
            $browsebyitem = true;
    ...
    */
    if ($auxFullFriendlyUrl && string_strpos($_GET["url_full"], ALIAS_LISTING_MODULE."/".ALIAS_CATEGORY_URL_DIVISOR)) {
        ...
        $sql = "SELECT id FROM ListingCategory WHERE full_friendly_url = ".db_formatString($aux_url_full)." AND enabled = 'y' LIMIT 1";
