	...
	function removeCategoryDropDown(type, url) {
		...
		if (type != 'banner' && type != 'promotion' && type != 'transaction' && type != 'invoice' && type != 'custominvoice') {
			try {
			...
			if (xmlhttp) {
				...
				xmlhttp.open('GET', url+'/'+SITEMGR_ALIAS+'/bulkupdate.php?ids=' + arrayIds + '&type=' + type + '&delete=' + document.getElementById('delete_all').checked, true);
				xmlhttp.send(null);
			}
		}
