...
<? if ($transaction_listing_log) { ?>
	...
	<table align="center" border="0" cellpadding="2" cellspacing="2" class="standard-innerTable">
		...
		<? foreach ($transaction_listing_log as $each_listing) { ?>
			<tr>
				<th>
					<fieldset title="<?=htmlspecialchars($each_listing["listing_title"])?>">
						<?
...
<? if ($transaction_case_log) { ?>

	<table align="center" border="0" cellpadding="2" cellspacing="2" class="standard-innerTable">
		<tr>
			<th class="tableTitle" colspan="4"><p>Cases</p></th>
		</tr>
		<? foreach ($transaction_case_log as $each_case) { ?>
			<tr>
				<th>
					<fieldset title="<?=$each_case["case_id"]?>">
						<?
						// $transactionListingObj = new Listing($each_listing["listing_id"]);
 ...
 ?>
						<?=system_showTruncatedText($each_case["case_id"], 60);?>
					</fieldset>
				</th>
				<td class="infoLevel"><?=system_showTruncatedText($each_case['reviewer_name'], 60)?></td>
				<td class="infoLevel"><?=system_showTruncatedText($each_case['review_title'], 60)?></td>
				<td class="infoAmount">
					<?=$each_case["amount"]." (".$transaction["transaction_currency"].")";?>
				</td>
			</tr>
		<? } ?>
	</table>
