...
//Case Unit
$sql1 = "select * from  {$dbObj->db_name}.Setting_Payment where name in ('PAYMENT_CURRENCY' , 'BRAINTREE_SUCCESS_EMAIL', 'BRAINTRE_FAIL_EMAIL')";
$resource = $dbObj->query( $sql1 );
...
<? if($transaction["transaction_status"] == "Complete"){ $transaction["transaction_status"] = "Success"; } ?>
	...
	<li><strong><?=system_showText(LANG_LABEL_DATE)?>:</strong><?php echo date('d M Y', strtotime($transac_date[0]))." - ";?><xyz id="here"></xyz></li>
	...
	<li><strong><?=system_showText(LANG_LABEL_NOTES)?>:</strong> <?
	if($transaction["notes"]) {
		echo "<p class = 'paymentnotes'>". str_replace('.',".<br>", $transaction["notes"]) . "</p>";	
	} else {
		echo " N/A ";
	}
...
<? if ($transaction_listing_log) {
	...
	<table border="0" cellpadding="2" cellspacing="2" class="table-itemlist">
		<tr>
			...
			<? if (PAYMENT_FEATURE == "on") { ?>
				...
				<? } ?>
			<? } ?>
			<th style="width:100px;"><?=system_showText(LANG_LABEL_RENEWAL);?></th>
		...
		<? foreach ($transaction_listing_log as $each_listing) { ?>
			<tr>
				...
				<? if (PAYMENT_FEATURE == "on") { ?>
					...
					<? } ?>
				<? } ?>
				<td style="text-align:center"><?=($each_listing["renewal_date"] == "0000-00-00") ? system_showText(LANG_NA) : date('d M Y', strtotime($each_listing["renewal_date"]))?></td>
...
<?php if( $transaction_case_log ): ?>
    <h2 class="standardSubTitle">Case Payments</h2>
    ...
    <table  class="table-itemlist">
        <tr>
            ...
            <th style="text-align:center">Case Opened Date</th>
            <th style="text-align:center">Promotional Code</th>
            <th style="text-align:center">Amount</th>
        </tr>
        <?php foreach ( $transaction_case_log as $case ): ?>
        <tr>
            <td style="text-align:center"><?=$case['case_id'];?></td>
            <td style="text-align:center"><?=$case['reviewer_name'];?></td>
            <td style="text-align:center"><?=$case['review_title'];?></td>
            <td style="text-align:center"><?=$case['opened_date'];?></td>
            <td style="text-align:center">
            <? if (trim($case['discount_id'])) { ?>
            	<?=$case['discount_id']?>
            <? } else { ?> N/A
            <?}?>
            </td>
            ...
            if (trim($transaction['transaction_currency'])){
            	$pryce .= $transaction['transaction_currency'];
            } else {
            	$pryce .= $currenc;
            }
            $pryce .= ")" ;
            ...
            ?>
            <td style="text-align:center"><?=$pryce?></td>
        </tr>
        <?php endforeach; ?>
    </table>
<?php endif; ?>
	...
	<table border="0" cellpadding="2" cellspacing="2" class="table-itemlist">
		...
		<? foreach ($transaction_custominvoice_log as $each_custominvoice) { ?>
			<tr>
				...
				?>
				<td><a href="<?=$popup_url?>id=<?=$each_custominvoice["custom_invoice_id"];?>&items=<?=urlencode($each_custominvoice["items"])?>&items_price=<?=urlencode($each_custominvoice["items_price"])?>&view=payment_log" class="iframe fancy_window_custom" style="text-decoration: underline;"><?=system_showText(LANG_VIEWITEMS)?></a></td>
	...
	<table border="0" cellpadding="2" cellspacing="2" class="table-itemlist">
		...
		<? foreach ($transaction_package_log as $each_package) { ?>
			<tr>
				...
				?>
				<td><a href="<?=$popup_url?>id=<?=$each_package["package_id"];?>&items=<?=urlencode($each_package["items"])?>&items_price=<?=urlencode($each_package["items_price"])?>&view=payment_log" class="iframe fancy_window_custom" style="text-decoration: underline;"><?=system_showText(LANG_VIEWITEMS)?></a></td>
				<td style="text-align:center; width: 100px;">
