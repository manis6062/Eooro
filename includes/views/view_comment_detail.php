<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /includes/views/view_comment_detail.php
	# ----------------------------------------------------------------------------------------------------

	$item_reviewcomment = "";

	$itemObj = new Post($commentArr[$i]["post_id"]);
    $postDetail = BLOG_DEFAULT_URL."/".$itemObj->getString("friendly_url");
	...
	$item_reviewcomment .= "<div class=\"comment ".$className."\">";
	...
	if (SOCIALNETWORK_FEATURE == "on") {
		 if ($commentArr[$i]["member_id"] && $account->getString("has_profile") == "y") {
			...
			$item_reviewcomment .= "<div class=\"image\"><span class=\"no-image\"></span></div>";
		 }
	...
	if (sess_getAccountIdFromSession()) {
        if ($reply_id && ($reply_id == $itemId) && !$success_message && !$success_approve_message) {
            $item_reviewcomment .= "&nbsp;&nbsp;|&nbsp;&nbsp;<a class=\"reply-comment\" href=\"javascript:void(0);\" onclick=\"cancelReply('".$postDetail."');\">".system_showText(LANG_CANCEL)."</a>";
        } else {
            ...
            $item_reviewcomment .= "<a class=\"reply-comment\" href=\"javascript:void(0);\" id=\"liCancelRId_$itemId\" style=\"display:none\" onclick=\"cancelReply('".$postDetail."');\">".system_showText(LANG_CANCEL)."</a>";
        }
	}
	$item_reviewcomment .= "</p>";
