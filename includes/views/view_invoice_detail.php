...
<h2 class="standardSubTitle"><?=system_showText(LANG_INVOICEINFO);?></h2>
	...
	<ul class="general-item">
		...
		<li><strong><?=system_showText(LANG_LABEL_ID);?>:</strong> <?=$invoice["id"]?><? if (string_strpos($url_base, "/".MEMBERS_ALIAS."")) { ?>&nbsp;<a href="<?=DEFAULT_URL?>/<?=MEMBERS_ALIAS?>/billing/invoice.php?id=<?=$id?>" class="iframe fancy_window_invoice"><img src="<?=DEFAULT_URL?>/images/icon_print.gif" border="0" alt="<?=system_showText(LANG_MSG_CLICK_TO_PRINT_INVOICE)?>" title="<?=system_showText(LANG_MSG_CLICK_TO_PRINT_INVOICE)?>" /></a><? } ?></li>
	...
	<table class="table-itemlist">
		...
		<? foreach($invoice_event as $each_invoice_event) { ?>
			<tr>
				...
				<? if (PAYMENT_FEATURE == "on") { ?>
					<? if ((CREDITCARDPAYMENT_FEATURE == "on") || (INVOICEPAYMENT_FEATURE == "on")) { ?>
						<td style="text-align:center"><?=$each_invoice_event["discount_id"]?></td>
					<? } ?>
				<? } ?>
				<td style="text-align:center"><?=$each_invoice_event["renewal_date"]?></td>
				<td style="text-align:center">
	...
	<table class="table-itemlist">
		...
		<? foreach($invoice_banner as $each_invoice_banner) { ?>
			<tr>
				...
				<td style="text-align:center"><?=$each_invoice_banner["impressions"]?></td>
				<td><?=string_ucwords($each_invoice_banner["level_label"]);?></td>
				<? if (PAYMENT_FEATURE == "on") { ?>
					<? if ((CREDITCARDPAYMENT_FEATURE == "on") || (INVOICEPAYMENT_FEATURE == "on")) { ?>
						<td style="text-align:center"><?=$each_invoice_banner["discount_id"]?></td>
					<? } ?>
				<? } ?>
				<td style="text-align:center"><?=$each_invoice_banner["renewal_date"]?></td>
				<td style="text-align:center">
	...
	<table class="table-itemlist">
		...
		<? foreach($invoice_classified as $each_invoice_classified) { ?>
			<tr>
				...
				<? if (PAYMENT_FEATURE == "on") { ?>
					<? if ((CREDITCARDPAYMENT_FEATURE == "on") || (INVOICEPAYMENT_FEATURE == "on")) { ?>
						<td style="text-align:center"><?=$each_invoice_classified["discount_id"]?></td>
					<? } ?>
				<? } ?>
				<td style="text-align:center"><?=$each_invoice_classified["renewal_date"]?></td>
				<td style="text-align:center">
	...
	<table class="table-itemlist">
		...
		<? foreach($invoice_article as $each_invoice_article) { ?>
			<tr>
				...
				<? if (PAYMENT_FEATURE == "on") { ?>
					<? if ((CREDITCARDPAYMENT_FEATURE == "on") || (INVOICEPAYMENT_FEATURE == "on")) { ?>
						<td style="text-align:center"><?=$each_invoice_article["discount_id"]?></td>
					<? } ?>
				<? } ?>
				<td style="text-align:center"><?=$each_invoice_article["renewal_date"]?></td>
				<td style="text-align:center">
	...
	<table class="table-itemlist">
		...
		<? foreach($invoice_custominvoice as $each_invoice_custominvoice) { ?>
			<tr>
				...
				?>
				<td><a href="<?=$popup_url?>id=<?=$each_invoice_custominvoice["custom_invoice_id"]?>&items=<?=urlencode($each_invoice_custominvoice["items"])?>&items_price=<?=urlencode($each_invoice_custominvoice["items_price"])?>&view=payment_log" class="iframe fancy_window_custom" style="text-decoration: underline;"><?=system_showText(LANG_VIEWITEMS)?></a></td>
	...
	<table border="0" cellpadding="2" cellspacing="2" class="table-itemlist">
		...
		<? foreach($invoice_package as $each_invoice_package) { ?>
			<tr>
				...
				?>
				<td><a href="<?=$popup_url?>id=<?=$each_invoice_package["package_id"]?>&items=<?=urlencode($each_invoice_package["items"])?>&items_price=<?=urlencode($each_invoice_package["items_price"])?>&view=payment_log" class="iframe fancy_window_custom" style="text-decoration: underline;"><?=system_showText(LANG_VIEWITEMS)?></a></td>
				<td style="text-align:center; width: 100px;">
