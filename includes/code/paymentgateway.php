	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if ($simplepay_recurringCheckbox || $paypal_recurringCheckbox || $linkpoint_recurringCheckbox || $authorize_recurringCheckbox) {
			...
			if (!payment_verifyItensRenewal($arrayItens)) {
                ...
                if ( $payment_sagepayStatus ) {
                    $params     = compact( 'sagepay_vendor', 'sagepay_username', 'sagepay_password', 'sagepay_integrationtype');
                    $result     = $sageApp->setTask( 'paymentsettings', 'verify', $params )->run( true );
                    
                    $arrayError .= $result ? '' : '50,';
                }
		...
		if (!$arrayError) {
			/**
                     * SagePay Payment
                     */
                    $params['sagepay_activation'] = $payment_sagepayStatus ? 'on' : 'off' ;
                    $sageApp->setTask( 'paymentsettings', 'setSettings', $params )->run();

        
        //BRAINTREE UPDATE		

        //BRAINTERE GENERAL VALUES
 			$sql_braintree_active = "UPDATE Setting_Payment SET value='".$braintree_active."' WHERE name='BRAINTREE_ACTIVE' LIMIT 1";
 			$result = $dbObj->query($sql_braintree_active);

 			$sql_braintree_environment = "UPDATE Setting_Payment SET value='".$braintree_environment."' WHERE name='BRAINTREE_ENVIRONMENT' LIMIT 1"; 
 			$result = $dbObj->query($sql_braintree_environment);

        //BRAINTREE SANDBOX VALUES

 			$sql_braintree_sb_merchantid = "UPDATE Setting_Payment SET value='".$braintree_sb_merchantid."' WHERE name='BRAINTREE_SB_MERCHANTID' LIMIT 1"; 
 			$result = $dbObj->query($sql_braintree_sb_merchantid);

 			$sql_braintree_sb_privatekey = "UPDATE Setting_Payment SET value='".$braintree_sb_privatekey."' WHERE name='BRAINTREE_SB_PRIVATEKEY' LIMIT 1"; 
 			$result = $dbObj->query($sql_braintree_sb_privatekey);

 			$sql_braintree_sb_publickey = "UPDATE Setting_Payment SET value='".$braintree_sb_publickey."' WHERE name='BRAINTREE_SB_PUBLICKEY' LIMIT 1"; 
 			$result = $dbObj->query($sql_braintree_sb_publickey);


        //BRAINTREE LIVE VERSION VALUES

 		 	$sql_braintree_merchantid = "UPDATE Setting_Payment SET value='".$braintree_merchantid."' WHERE name='BRAINTREE_MERCHANTID' LIMIT 1"; 
 			$result = $dbObj->query($sql_braintree_merchantid);

 			$sql_braintree_privatekey = "UPDATE Setting_Payment SET value='".$braintree_privatekey."' WHERE name='BRAINTREE_PRIVATEKEY' LIMIT 1"; 
 			$result = $dbObj->query($sql_braintree_privatekey);

 			$sql_braintree_publickey = "UPDATE Setting_Payment SET value='".$braintree_publickey."' WHERE name='BRAINTREE_PUBLICKEY' LIMIT 1"; 
 			$result = $dbObj->query($sql_braintree_publickey);
		...
		if ($arrayError) {
			...
			$modules = array("currency", "item", "payment", "paypal", "simplepay", "creditcard", "pagseguro", "sagepay");
			$aux_module = explode("_", $settingSection);
			$module = array_search($aux_module[0], $modules);
			if ($module) $paramRedirect = "&module=$module";
			else $paramRedirect = "";
			header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/prefs/paymentgateway.php?arrayError=".$arrayError.$paramRedirect);
		} else {
            ...
            todo_updateItem("todo_paymentgateway");
			header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/prefs/paymentgateway.php?success_payment=true$paramRedirect");
		}
		exit;
	} else {
		...
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "SIMPLEPAY_ACCESSKEY") {
                $simplepay_accesskey = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "SIMPLEPAY_SECRETKEY") {
                $simplepay_secretkey = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "SIMPLEPAY_STATUS") {
                $payment_simplepayStatus = $row["value"];
            }
			if ($row["name"] == "SIMPLEPAY_RECURRING") {
                $simplepay_recurringCheckbox = $row["value"];
            }
			if ($row["name"] == "SIMPLEPAY_RECURRINGCYCLE") {
                $simplepay_recurringcycle = $row["value"];
            }
			if ($row["name"] == "SIMPLEPAY_RECURRINGTIMES") {
                $simplepay_recurringtimes = $row["value"];
            }
			if ($row["name"] == "SIMPLEPAY_RECURRINGUNIT") {
                $simplepay_recurringunit = $row["value"];
            }
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "PAYPAL_ACCOUNT") {
                $paypal_account = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "PAYPAL_STATUS") {
                $payment_paypalStatus = $row["value"];
            }
			if ($row["name"] == "PAYPAL_RECURRING") {
                $paypal_recurringCheckbox = $row["value"];
            }
			if ($row["name"] == "PAYPAL_RECURRINGCYCLE") {
                $paypal_recurringcycle = $row["value"];
            }
			if ($row["name"] == "PAYPAL_RECURRINGTIMES") {
                $paypal_recurringtimes = $row["value"];
            }
			if ($row["name"] == "PAYPAL_RECURRINGUNIT") {
                $paypal_recurringunit = $row["value"];
            }
		}

		//BRAINTREE

		$sql = "SELECT * FROM Setting_Payment WHERE name LIKE 'BRAINTREE%'";
		$result = $dbObj->query($sql);
		
		while($row = mysql_fetch_assoc($result) ){
				if($row['name'] == "BRAINTREE_ACTIVE") {
					  $braintree_active = $row["value"];
				}
				if($row['name'] == "BRAINTREE_ENVIRONMENT") {
					  $braintree_environment = $row["value"];
				}    
				if($row['name'] == "BRAINTREE_MERCHANTID") {
					  $braintree_merchantid = $row["value"];
				}    
				if($row['name'] == "BRAINTREE_PRIVATEKEY") {
					  $braintree_privatekey = $row["value"];
				}  
				if($row['name'] == "BRAINTREE_PUBLICKEY") {
					  $braintree_publickey = $row["value"];
				}  
				if($row['name'] == "BRAINTREE_SB_MERCHANTID") {
					  $braintree_sb_merchantid = $row["value"];
				}  
				if($row['name'] == "BRAINTREE_SB_PRIVATEKEY") {
					  $braintree_sb_privatekey = $row["value"];
				}  
				if($row['name'] == "BRAINTREE_SB_PUBLICKEY") {
					  $braintree_sb_publickey = $row["value"];
				}  
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "PAYPALAPI_STATUS") {
                $payment_paypalapiStatus = $row["value"];
            }
			if ($row["name"] == "PAYPALAPI_USERNAME") {
                $paypalapi_username = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "PAYPALAPI_PASSWORD") {
                $paypalapi_password = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "PAYPALAPI_SIGNATURE") {
                $paypalapi_signature = crypt_decrypt($row["value"]);
            }
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "PAYFLOW_STATUS") {
                $payment_payflowStatus = $row["value"];
            }
			if ($row["name"] == "PAYFLOW_LOGIN") {
                $payflow_login = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "PAYFLOW_PARTNER") {
                $payflow_partner = crypt_decrypt($row["value"]);
            }
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "TWOCHECKOUT_STATUS") {
                $payment_twocheckoutStatus = $row["value"];
            }
			if ($row["name"] == "TWOCHECKOUT_LOGIN") {
                $twocheckout_login = crypt_decrypt($row["value"]);
            }
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "PSIGATE_STATUS") {
                $payment_psigateStatus = $row["value"];
            }
			if ($row["name"] == "PSIGATE_STOREID") {
                $psigate_storeid = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "PSIGATE_PASSPHRASE") {
                $psigate_passphrase = crypt_decrypt($row["value"]);
            }
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "WORLDPAY_STATUS") {
                $payment_worldpayStatus = $row["value"];
            }
			if ($row["name"] == "WORLDPAY_INSTID") {
                $worldpay_instid = crypt_decrypt($row["value"]);
            }
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "ITRANSACT_STATUS") {
                $payment_itransactStatus = $row["value"];
            }
			if ($row["name"] == "ITRANSACT_VENDORID") {
                $itransact_vendorid = crypt_decrypt($row["value"]);
            }
		}		
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "LINKPOINT_CONFIGFILE") {
                $linkpoint_configfile = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "LINKPOINT_KEYFILE") {
                $linkpoint_keyfile = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "LINKPOINT_STATUS") {
                $payment_linkpointStatus = $row["value"];
            }
			if ($row["name"] == "LINKPOINT_RECURRING") {
                $linkpoint_recurringCheckbox = $row["value"];
            }
			if ($row["name"] == "LINKPOINT_RECURRINGTYPE") {
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "AUTHORIZE_LOGIN") {
                $authorize_login = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "AUTHORIZE_TXNKEY") {
                $authorize_txnkey = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "AUTHORIZE_STATUS") {
                $payment_authorizeStatus = $row["value"];
            }
			if ($row["name"] == "AUTHORIZE_RECURRING") {
                $authorize_recurringCheckbox = $row["value"];
            }
			if ($row["name"] == "AUTHORIZE_RECURRINGLENGTH") {
                $authorize_recurringlength = $row["value"];
            }
		}
		...
		$result = $dbObj->query($sql);
		while ($row = mysql_fetch_assoc($result)) {
			if ($row["name"] == "PAGSEGURO_EMAIL") {
                $pagseguro_email = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "PAGSEGURO_TOKEN") {
                $pagseguro_token = crypt_decrypt($row["value"]);
            }
			if ($row["name"] == "PAGSEGURO_STATUS") {
                $payment_pagseguroStatus = $row["value"];
            }
		}
