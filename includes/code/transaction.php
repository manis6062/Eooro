<?
	...
	***************************************************************/
	...
	if (isset($transaction_package_log))       unset($transaction_package_log);
        // case modification
        if ( isset($transaction_case_log) )         unset( $transaction_case_log );
	...
	if ($id = $_GET["id"]){
		...
		if ($transactionObj) {
			...
			//Case Subtotal is total in sagepay(disconunt id) Subtotal = total - tax
			$transaction["transaction_subtotal"] = $transactionObj->getString("transaction_subtotal");
			
			//Sage PAY MODIFICATION

			if ($transactionObj->system_type == "sagepay") {
				$transaction["transaction_tax"]	 = $transactionObj->transaction_tax;
				$transaction["transaction_subtotal"] = $transactionObj->getString("transaction_amount") - $transactionObj->getString("transaction_tax");
				$transaction["transaction_subtotal"] = number_format($transaction["transaction_subtotal"], 2);
			} else {
			...
			$i++;
                         ...
                         * Case Modification
                         */
                        $sql = "SELECT payment_log_id, case_id, amount FROM Payment_Case_Log WHERE payment_log_id='{$transaction['payment_log_id']}'";
                        $r   = $db->query( $sql );
                        $i = 0;
                        while( $row    = mysql_fetch_assoc($r) ){
                            $caseObj   = new Opened_Cases( $row['case_id']);
                            $reviewObj = new Review( $caseObj->getNumber('review_id') );
                            
                            $transaction_case_log[$i]['case_id']        	= $row['case_id'];
                            $transaction_case_log[$i]['reviewer_name']  	= $reviewObj->getString( 'reviewer_name' );
                            $transaction_case_log[$i]['review_title']   	= $reviewObj->getString( 'review_title' );
                            $transaction_case_log[$i]['opened_date']    	= format_date( $caseObj->getString( 'opened_date' ) );
                            $transaction_case_log[$i]['discount_id']		= $caseObj->getString( 'discount_id' );
                            $transaction_case_log[$i]['transaction_tax']    = payment_calculateTax($transactionObj->getString("transaction_subtotal"),$transactionObj->getString("transaction_tax"),true,false);
                            $transaction_case_log[$i]['amount']         	= number_format($row['amount'], 2);
                          $i++;
                        }
