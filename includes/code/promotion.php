<?
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (validate_form("promotion", $_POST, $message_promotion) && ($upload_image != "failed")) {
            ...
            if ($upload_image != "failed" && !$crop_submit) {
                ...
                header("Location: ".$url_base."/".PROMOTION_FEATURE_FOLDER."/".(($search_page) ? "search.php" : "index.php")."?module=promotion&message=".$message.(($id) ? "" : "&newest=1")."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "").(($id) ? "" : "&extra_message=1"));
                exit;
    ...
    if ($id) {
        ...
        if ($deal_type == "percentage") {
            $deal_price_int = round(100 - ((100 * $dealvalue) / $realvalue));
        }
    } else {
