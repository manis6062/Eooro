<?
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (validate_form("backlink", $_POST, $message_backlink)) {
            ...
            if ($sitemgr){
                ...
                $listingObj->setString("backlink_url", $backlink_url);
				$listingObj->setString("custom_text0", $wigType);
    ...
    if ($id){
        ...
        $backlinks = "<a id=\"eooro-widget-backlink\" href=\"";
        if(!empty($listingObj->custom_dropdown5)){
         $backlinks .= LISTING_DEFAULT_URL."/".$listingObj->getString("custom_dropdown5");
        }
        else{
       $backlinks .= LISTING_DEFAULT_URL."/".$listingObj->getString("friendly_url");
        }
