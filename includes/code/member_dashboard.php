<?
    ...
    switch ($item_type) {
        case "Listing" :
                        ...
                        // modification
                        $reply_review = $levelObj->getReplyReview( $itemObj->getNumber("level") );
                        $open_case = $levelObj->getOpenCase( $itemObj->getNumber("level") );
                        break;
        case "Event" :
        ...
        case "Promotion" :
                        ...
                        break;
        case "Banner" :
                        $item_link = DEFAULT_URL."/".MEMBERS_ALIAS."/".BANNER_FEATURE_FOLDER."/edit.php?id=".$itemObj->getNumber("id");
                        $fieldTitle = "caption";
                        $review_enabled = "off";
        ...
        if (strtolower($item_type) == "banner" && $itemObj->getString("expiration_setting") == BANNER_EXPIRATION_IMPRESSION) {
            ...
            if ($renewal_date) {
                ...
                */
                ...
                $item_expired = ($itemObj->getString("status") == 'E' ) ? true: false;
                /*
                ...
                */
                ...
                if ($diffdays > 365) {
                    ...
                    $val = $diffdays - (365*$years);   
                    $val1 = floor($val/30); 
                    if($val1!=0){
                            $month1 = $val1; 
                            if($month1==1){ $month_period = system_showText(LANG_MONTH); $item_renewal = $years;}
                            elseif($month1==12){ $item_renewal = $years + 1; $month1=""; }
                            else{
                                $month_period = system_showText(LANG_MONTH_PLURAL); $item_renewal = $years; 
                            }
                        }    
    ...
    if (strtolower($item_type) != "banner") {
        ...
        if (($levelObj && $levelObj->getDetail($itemObj->getNumber("level")) == "y") || strtolower($item_type) == "promotion") {
            $linkTwitter = $moduleURL."/".$itemObj->getString("friendly_url");
            $linkFacebook = $moduleURL."/".ALIAS_SHARE_URL_DIVISOR."/".$itemObj->getString("friendly_url");
        } else {
    ...
    if ($review_enabled == "on" && $commenting_edir) {
        ...
        // Page Browsing /////////////////////////////////////////   
        $where = "Review.item_type = '".strtolower($item_type)."' AND Review.item_id = '$item_id' AND Review.item_id = $reviewTable.id AND $reviewTable.account_id = '$acctId' AND is_deleted=0";
    ...
    if (is_array($array_fields) && (in_array("phone", $array_fields) || in_array("contact_phone", $array_fields))) {
        $item_hasphone = true;
    }
