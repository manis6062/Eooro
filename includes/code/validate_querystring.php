<?
	...
	if ($_GET["keyword"]) {
		...
		//$_GET["keyword"] = htmlspecialchars_decode(trim($_GET["keyword"])); //dont sanitize here(but in query)
        // $_GET["keyword"] = system_denyInjections(trim($_GET["keyword"])); //earlier used (it truncates the script tag resulting in listing not be found while search) 
	}
	if ($_POST["keyword"]) {
