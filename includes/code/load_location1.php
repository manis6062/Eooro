    ...
    if ($_non_default_locations) {
        ...
        foreach ($_non_default_locations as $_location_level) {
            ...
            <div id="div_location_<?= $_location_level ?>" <?= (${"locations" . $_location_level} && ($_location_father_level !== false ? ${"location_" . $_location_father_level} : true) || ($has_default_location && $_location_level == ($last_default_location + 1) && ${"locations" . $_location_level}) ) ? "" : "style=\"display:none;\"" ?>>
                ...
                <select tabindex="11" class="select" name="location_<?= $_location_level ?>" id="location_<?= $_location_level ?>" <? if ($_location_child_level) { ?> onchange="loadLocation('<?= DEFAULT_URL ?>', '<?= EDIR_LOCATIONS ?>', <?= $_location_level ?>, <?= $_location_child_level ?>, this.value, <?= ($firstLoc ? "true" : "false") ?>, <?= ($newLocStyle ? "1" : "0") ?>);" <? } elseif (!$newLocStyle) { ?> onchange="fillLocations('<?= EDIR_LOCATIONS ?>')"<? } ?>>

                    <option id="l_location_<?= $_location_level ?>" value="0"><?= system_showText(constant("LANG_SEARCH_LABELCB" . constant("LOCATION" . $_location_level . "_SYSTEM"))) ?></option>
    ...
    <div id="formsLocation" class="form-location ">
        <table cellpadding="0" cellspacing="0" border="0" class="standard-table standardSIGNUPTable businessLocationTable <?= $contact ? "noMargin" : ""; ?>"> <?
            if ($_default_locations_info) {
             ...
             if ($_non_default_locations) { 
                foreach ($_non_default_locations as $key => $_location_level) {
                    ...
                    if ($location_name == "Country") {
                        $location_name = "Country";
                            //if only country is present and not state
                            if(${"locations" . $_location_level} && empty(${"locations" . $_non_default_locations[$key+1]})) {
                                $location = 'location_'.$_location_level; 
                                $countryId = $listing->$location;
                                ${"locations" . $_non_default_locations[$key+1]} = Location3::retrieveAllLocationByLocation1($countryId);                            
                            }

                    } elseif ($location_name == "State") {
                        $location_name = "County/State";
                            //if only country and state is present
                            if(${"locations" . $_location_level} && empty(${"locations" . $_non_default_locations[$key+1]})) {
                                $location = 'location_'.$_location_level; 
                                $stateId = $listing->$location;
                                $cityList = Location4::retrieveAllLocationByLocation1($countryId, $stateId);                            
                            }
                    ...
                    <tr id="div_location_<?= $_location_level ?>" <?= ((${"locations" . $_location_level} & $_POST["new_location" . $_location_level . "_field"] == "") || (!${"locations" . $_location_level} && ${"location_" . $_location_father_level} && !$_POST["new_location" . $_location_level . "_field"])) ? "" : "style=\"display:none\"" ?>>
                        <th>
                            <label for="location_<?= $_location_level ?>" class="location"><?= $location_name ?>:
                                <? if ($_GET['id']) {
                                    if ($listing->custom_checkbox1 == "n") { 
                                        $as_style = "style = 'display : inline-block'"; 
                                        } 
                                        else $as_style = "style = 'display : none'";
                                    }
                                else{
                                    $as_style = "style = 'display : inline-block'"; 
                                }?>
                        ...
                        <td id="div_select_<?= $_location_level ?>" class="field locationSelect">
                            <? if ($use_sitemgr_function) { //echo $use_sitemgr_function;?>
                                <select tabindex="12" <?= ((${"locations" . $_location_level}) ? "" : "style=\"display:none\"") ?> class="select <?= ($highlight == "description" && !${"location_" . $_location_level} ? "highlight" : "") ?>" name="<?= ($sitemgrSearch ? "search_" : "") ?>location_<?= $_location_level ?>" id="location_<?= $_location_level ?>" <? if ($_location_child_level) { ?> onchange="loadLocationSitemgrMembers('<?= DEFAULT_URL ?>', '<?= EDIR_LOCATIONS ?>', <?= $_location_level ?>, <?= $_location_child_level ?>, this.value); <? if ($loadMap) { ?> loadMap(<?= $formLoadMap ?>); <? } ?>" <? } elseif ($loadMap) { ?> onchange="loadMap(<?= $formLoadMap ?>);" <? } ?> <?= (($listing->status == "A" && !$claimlistingid && $_location_level == 1) ? "disabled" : "") ?>>
                                <? } else { ?>
                                    <select tabindex="13" <?= ((${"locations" . $_location_level}) ? "" : "style=\"display:none\"") ?> class="select" name="<?= ($sitemgrSearch ? "search_" : "") ?>location_<?= $_location_level ?>" id="location_<?= $_location_level ?>" <? if ($_location_child_level) { ?> onchange="loadLocation('<?= DEFAULT_URL ?>', '<?= EDIR_LOCATIONS ?>', <?= $_location_level ?>, <?= $_location_child_level ?>, this.value);" <? } ?> <?= ($id ? "disabled" : "") ?>>
                                    <? } ?>
                                    ...
                                    if (is_array(${"locations" . $_location_level})) {
                                        foreach (${"locations" . $_location_level} as $each_location) {
                                            $selected = ($listing->$location == $each_location["id"]) ? "selected" : "";
                                            ?><option <?= $selected ?> value="<?= $each_location["id"] ?>"><?= $each_location["name"] ?></option><?
                                            unset($selected);
                                        }
                                    } elseif($cityList) { //for city dropdown only
                                        foreach ($cityList as $each_location) {
                                            $selected = ($listing->$location == $each_location["id"]) ? "selected" : "";
                                            ?><option <?= $selected ?> value="<?= $each_location["id"] ?>"><?= $each_location["name"] ?></option><?
                                            unset($selected);
                                        }
