<?
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("mobilenotif", $_POST, $message_notification)) {
			...
			header("Location: $url_redirect/notifications.php?process=".$process."&newest=".$newest."&message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : ""));
			exit;
	...
	$statusDropDown = html_selectBox("status", $arrayNameDD, $arrayValueDD, $status, "", "style='width: 200px;'", "-- ".system_showText(LANG_LABEL_SELECT_ALLSTATUS)." --");
