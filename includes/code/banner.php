		...
		if ((validate_form("banner", $_POST, $val_message, $error_size)) && is_valid_discount_code($_POST["discount_id"], "banner", $_POST["id"], $val_message, $discount_error_num)) {
			...
			if ($_POST["account_id"] > 0) {
				...
				if($emailNotificationObj = system_checkEmail(SYSTEM_NEW_BANNER)) {
					...
					system_mail($contactObj->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contactObj->account_id, SYSTEM_NEW_BANNER);
				}
			}
	...
	****************************************************************************/
	if ($operation == "update") {
		...
		if ((validate_form("banner", $_POST, $val_message, $error_size)) && is_valid_discount_code($_POST["discount_id"], "banner", $_POST["id"], $val_message, $discount_error_num)) {
			...
			header("Location: ".$url_redirect."/".(($search_page) ? "search.php" : "index.php")."?module=banner&process=".$process."&newest=".$newest."&message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
			exit;
