	...
	if ($_SERVER['REQUEST_METHOD'] == "POST" && ($hiddenValue == "Submit" || $bulkSubmit == "Submit" || $bulkSubmit == LANG_SITEMGR_SUBMIT)) {
		...
		if ($ids) {
			if ($delete_all == "on") {
				...
				if (!$change_section && !$add_category_id && $typeName == "Banner" && !$change_no_owner && !$change_account_id && !$status && !$change_renewaldate) {
					$error_message = 1;
			...
			if (string_strpos($_SERVER["PHP_SELF"], "search.php")) {
				...
				if ($error_message || $error_msg) {
					...
					if ($success_delete) {
						header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".string_strtolower($typeName)."/index.php?msg=successdel&letter=$letter&screen=$screen");
					} else {
						header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/".string_strtolower($typeName)."/index.php?msg=success&letter=$letter&screen=$screen");
					}
					exit;
				}
			}
			...
			if ($bulkSubmit) {
				if ($delete_all) {
					$error_message = 2;
				} else if (!$change_section && !$change_section && !$add_category_id && $typeName == "Banner" && !$change_no_owner && !$change_account_id && !$status && !$change_renewaldate && !$add_category_id) {
					$error_message = 4;
				} else if (!$change_no_owner && !$change_account_id && !$status && !$level && !$change_renewaldate && !$add_category_id && !$return_categories && !$removecategory && $typeName != "Banner") {
					$error_message = 4;
				} else {
