<?
    ...
    if (string_strpos($_SERVER["PHP_SELF"], "listing")) {
        $feature_submenu = "listing";
    } elseif (string_strpos($_SERVER["PHP_SELF"], "event")) {
        $feature_submenu = "event";
    } elseif (string_strpos($_SERVER["PHP_SELF"], "classified")) {
        $feature_submenu = "classified";
    } elseif (string_strpos($_SERVER["PHP_SELF"], "article")) {
        $feature_submenu = "article";
    } elseif (string_strpos($_SERVER["PHP_SELF"], "blog")) {
        $feature_submenu = "blog";
    }
    ...
    <div class="submenu">
        <ul>
            <li id="privateMenu_home"><a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=$feature_submenu?>categs/index.php"><?=system_showText(LANG_SITEMGR_MANAGE)?></a></li>
            <li id="privateMenu_add"><a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=$feature_submenu?>categs/category.php<?=(isset($category_id)?'?category_id='.$category_id:'')?>"><?=system_showText(LANG_SITEMGR_MENU_ADD)?></a></li>
            <? if ($featuredcategory_actived == "on") { ?>
                <li id="privateMenu_featured"><a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=$feature_submenu?>categs/featured.php"><?=system_showText(LANG_SITEMGR_FEATURED)?></a></li>
            <? } ?>
            <li id="privateMenu_disabled"><a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/<?=$feature_submenu?>categs/disabled.php<?=(isset($category_id)?'?category_id='.$category_id:'')?>"><?=system_showText(LANG_SITEMGR_DISABLED2)?></a></li>
            <?
