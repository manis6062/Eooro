<?
    ...
    if ($aux_modal_box) {
		...
		<div>
			...
			<input class="text" type="email" name="username" id="username" value="<?=$defaultusername?>" required/>
		</div>
            
		<div>
			<label for="password"><?=system_showText(LANG_LABEL_PASSWORD);?>:</label>
			<input class="text" type="password" name="password" id="password" value="<?=$defaultpassword?>" required/>
		</div>
	...
	<? } elseif (!$sitemgr_section) {
		...
		<div class="form-login">
			<label for="username"><?=system_showText(LANG_LABEL_USERNAME);?></label>
			<input class="span12" type="email" name="<?=$advertise_section ? "dir_" : ""?>username" id="<?=$advertise_section ? "dir_" : ""?>username" value="<?=$defaultusername?>" required/>
			<span class="clear"></span>
			<label for="password"><?=system_showText(LANG_LABEL_PASSWORD);?></label>
			<input class="span12" type="password" name="<?=$advertise_section ? "dir_" : ""?>password" id="<?=$advertise_section ? "dir_" : ""?>password" value="<?=$defaultpassword?>" required/>
		</div>
		
        <? if ($automatically !== false) { ?>
            <div class="checkbox">
                <label>
                    <input type="checkbox" name="automatic_login" value="1" <?=$checked?> class="checkbox" />
                    <?=system_showText(LANG_AUTOLOGIN);?>
                </label>
            </div>
        <? } ?>
        ...
        <div class="row-fluid action">
            ...
            <div class=" pull-left span6">					
                <? if (system_checkEmail(SYSTEM_FORGOTTEN_PASS)) { ?>
                    <p class="forgotpassword"><a href="<?=$forgotLink;?>"><?=system_showText(LANG_LABEL_FORGOTPASSWORD);?></a></p>
                <? }
                if (DEMO_MODE) { ?> <small><?=system_showText(LANG_LABEL_TESTPASSWORD)?>: <b>abc123</b> </small><? } ?>
            </div>

            <button class="btn btn-login pull-right span6" <?=($advertise_section ? "type=\"button\"  onclick=\"submitForm('formDirectory');\"" : "type=\"submit\"")?>><?=system_showText(LANG_BUTTON_LOGIN);?></button>
