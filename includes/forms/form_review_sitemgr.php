...
<table class="table-itemform">
	...
	<tr>
		...
		<td class="alignTop"><input type="text" name="reviewer_name" id="reviewer_name<?=$each_rate->getNumber('id');?>" value="<?=$each_rate->getString("reviewer_name", true, 0, "...", false);?>" maxlength="50" class="input-form-rate" /></td>
	</tr>
	<tr>
		<th class="reviewTh"><?=system_showText(LANG_SITEMGR_LABEL_EMAIL)?>:</th>
		<td class="alignTop"><input type="text" name="reviewer_email" id="reviewer_email<?=$each_rate->getNumber('id');?>" value="<?=$each_rate->getString("reviewer_email", true, 0, "...", false);?>" maxlength="100" class="input-form-rate" /></td>
	</tr>
	<tr>
		<th class="reviewTh"><?=system_showText(LANG_SITEMGR_LABEL_CITY_STATE)?>:</th>
		<td class="alignTop"><input type="text" name="reviewer_location" id="reviewer_location<?=$each_rate->getNumber('id');?>" value="<?=$each_rate->getString("reviewer_location", true, 0, "...", false);?>" maxlength="50" class="input-form-rate" /></td>
	</tr>
	<tr>
		<th class="reviewTh"><?=system_showText(LANG_SITEMGR_TITLE)?>:</th>
		<td class="alignTop"><input type="text" name="review_title" id="review_title<?=$each_rate->getNumber('id');?>" value="<?=$each_rate->getString("review_title", true, 0, "...", false);?>" maxlength="50" class="input-form-rate" /></td>
	</tr>
	<tr>
		<th class="reviewTh"><?=system_showText(LANG_SITEMGR_LABEL_COMMENT)?>:</th>
		<td class="alignTop"><textarea name="review" id="review<?=$each_rate->getNumber('id');?>" class="input-textarea-form-rate" rows="5"><?=$each_rate->getString("review", true, 0, "...", false);?></textarea></td>
	</tr>
	</tbody>
</table>
