<?
    ...
    if ($message) { ?>
        <p class="content-custom alert <?=$message_style?>"> <?=$message?></p>
    <? }
    ...
    if ((string_strlen(trim($message_member)) > 0) || (string_strlen(trim($message_account)) > 0) || (string_strlen(trim($message_contact)) > 0) ) { ?>
        ...
        <p class="errorMessage">
            ...
            <? if (string_strlen(trim($message_account)) > 0) { ?>
                 <?if($message_account!="&#149;&nbsp;E-mail is required."){?>
                 <?=$message_account?>
                 <? }?>
    ...
    <p class="successMessage alert alert-success" id="messageEmail" style="display:none"><?=system_showText(LANG_LABEL_ACTIVATEEMAIL_SENT);?></p>
    <p class="errorMessage" id="messageEmailError" style="display:none"></p>
    
    <? if ($messageAct) { ?>
        <p class="successMessage"><?=system_showText(LANG_MSG_ACCOUNT_ACTIVATED);?></p>
    <? } ?>