...
<table id="form-smtp" cellpadding="2" cellspacing="0" border="0" class="standard-table noMargin">
	...
	<tr>
		<th><?=system_showText(LANG_SITEMGR_SETTINGS_EMAILCONF_LABEL_SERVER)?>:</th>
		<td>
			<input type="text" name="emailconf_host" id="host" <? if ($emailconf_host) echo "value=\"$emailconf_host\""; ?> <?=$styleButtonChange?> />&nbsp;<?=system_showText(LANG_SITEMGR_SETTINGS_EMAILCONF_LABEL_PORT)?>:&nbsp;
	...
	<tr>
		<th><input class="standard-table-putradio" type="radio" name="emailconf_auth" id="auth1" value="normal" <?=$styleButtonClick?> /></th>
		<td>
	...
	<tr>
		<th>
            <input class="standard-table-putradio" type="radio" name="emailconf_auth" id="auth2" value="secure" <?=$styleButtonClick?> />
        </th>
		<td>
            <label for="auth2">
                <?=system_showText(LANG_SITEMGR_SETTINGS_EMAILCONF_SERVERREQUIRESAUTHENTICATION2)?>                
            </label>
	...
	<tr>
		<th><input class="standard-table-putradio" type="radio" name="emailconf_auth" id="auth3" value="noauth" <?=$styleButtonClick?> /></th>
		<td>
	...
	<tr>
		...
		<td>
			<input type="text" name="emailconf_email" id="email" <? if ($emailconf_email) echo "value=\"$emailconf_email\""; ?> onkeyup="emailChange(this.value)" onkeypress="emailChange(this.value)" onblur="emailBlur(this.form)" <?=$styleButtonChange?> />&nbsp;<span id="email_status" style="display:inline"></span>
		</td>
	</tr>
	<tr>
		<th><?=system_showText(LANG_SITEMGR_USERNAME)?>:</th>
		<td>
			<input type="text" name="emailconf_username" id="username" <? if ($emailconf_username) echo "value=\"$emailconf_username\""; ?>  <?=$styleButtonChange?> />
		</td>
	</tr>
	<tr>
		<th><?=system_showText(LANG_SITEMGR_LABEL_PASSWORD)?>:</th>
		<td>
			<input type="password" name="emailconf_password" id="password"  <?=$styleButtonChange?> />
		</td>
	</tr>
	<tr>
		<th></th>
