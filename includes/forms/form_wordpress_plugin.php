...
?>
	<script language="javascript" type="text/javascript">
		function download_wordpress_plugin(type){
			<? if (!DEMO_LIVE_MODE) { ?>
				document.location = "<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/plugins/index.php?download=1&type="+type;
			<? } else { ?>
			...
			function(){
				$("#wordpress_plugin").submit(function(event){
                    ...
                    if ($("#wordpress_url").val()){
                        ...
                        $.post("<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/plugins/wordpress_plugin_ajax.php", $("#wordpress_plugin").serialize(), 
                                function(data){
	...
	<div class="wordPressPlugIn">
		<img src="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/images/design/img_wordpress_logo.png" />
