...
?>
	<script language="javascript" type="text/javascript">
		function download_sugar_plugin(){
			<? if (!DEMO_LIVE_MODE) { ?>
				document.location = "<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/plugins/index.php?download=1&type=sugar";
			<? } else { ?>
			...
			function(){
				$("#sugar_plugin").submit(function(event){
                    ...
                    if ($("#sugar_url").val() && $("#sugar_user").val() && $("#sugar_password").val()){
                        ...
                        $.post("<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/plugins/sugarCRM_plugin_ajax.php", $("#sugar_plugin").serialize(), 
                        function(data){
	...
	<div class="sugarCRMPlugIn">
		<img src="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/images/design/img_sugarcrm_logo.gif" />
		<div class="sugarLeft">
