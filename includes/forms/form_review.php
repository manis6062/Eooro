<?
	...
	<div class="left">
		...
		<div>
			...
			<input class="text" type="text" name="reviewer_name" id="reviewer_name" value="<?=(sess_getAccountIdFromSession() && SOCIALNETWORK_FEATURE == "on" && $reviewerAcc->getString("has_profile") == "y") ? ($reviewerProfile->getString("nickname")) : ($reviewer_name)?>" maxlength="50" tabindex="1" />
		</div>
		<div>
			<label>* <?=system_showText(LANG_LABEL_COMMENTTITLE);?>:</label>
			<input class="text" type="text" name="review_title" id="review_title" value="<?=$review_title?>" maxlength="20" tabindex="2" />
		</div>
		<div>
	...
	<div class="action">
		...
		<button type="submit" name="submit" value="Submit" id="submitReview"><?=system_showText(LANG_BUTTON_SEND)?></button>
