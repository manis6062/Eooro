<?
	...
	if ($array_events) {
		...
		<h2>
			...
			<a class="view-more" href="<?=EVENT_DEFAULT_URL?>/"><?=$seeAllText;?></a>
		</h2>
			...
			foreach ($array_events as $event) {
				...
				echo "<div class=\"featured-item ".(($lastItemStyle==$numberOfEvents) ? ("pd-0") : (""))."\">";
					...
					if ($imageObj->imageExists()) {
						...
						echo $imageObj->getTag(true, IMAGE_FRONT_EVENT_WIDTH, IMAGE_FRONT_EVENT_HEIGHT, $event->getString("title", false), true);
						echo "</a>";
					} else {
						echo "<a href=\"".$detailLink."\" class=\"image\">";
