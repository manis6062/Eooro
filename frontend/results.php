<?
		...
		if ($listingsRelatedSearch > 0) {
			...
			<ul class="list list-category">
				...
				<li class="level-1 level-title">
					<a rel="canonical"  href="<?=LISTING_DEFAULT_URL?>/results.php?keyword=<?=$keyword?>&amp;where=<?=$where?>"><?=system_showText(LANG_MENU_LISTING);?></a> <span>(<?=$listingsRelatedSearch?>)</span>
				</li>
				
				<?

				if (CATEGORY_SCALABILITY_OPTIMIZATION != "on") {
					$categories = db_getFromDBBySQL("listingcategory", "SELECT * FROM ListingCategory WHERE category_id = 0 AND enabled = 'y' ORDER BY $orderbyfield");
					if ($categories) {
						...
						foreach ($categories as $category) {
							...
							if ($thislistings > 0) {
								?>
								<li class="level-2">
									<span class="level-1 level-title">&nbsp;</span><a rel="canonical" href="<?=LISTING_DEFAULT_URL?>/results.php?keyword=<?=$keyword?>&amp;where=<?=$where?>&amp;category_id=<?=$this_category_id?>"><?=$category->getString("title")?></a> <span>(<?=$thislistings?>)</span>
								</li>
								<?
							}
		...
		if (EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on") {
			...
			if ($eventsRelatedSearch > 0) {
				...
				<ul class="list list-category">
					...
					<?
					if (CATEGORY_SCALABILITY_OPTIMIZATION != "on") {
						$categories = db_getFromDBBySQL("eventcategory", "SELECT * FROM EventCategory WHERE category_id = 0 AND enabled = 'y' ORDER BY $orderbyfield");
						if ($categories) {
							...
							foreach ($categories as $category) {
								...
								if ($thisevents > 0) {
									?>
									<li class="level-2">
										<a href="<?=EVENT_DEFAULT_URL?>/results.php?keyword=<?=$keyword?>&amp;where=<?=$where?>&amp;category_id=<?=$this_category_id?>"><?=$category->getString("title")?></a> <span>(<?=$thisevents?>)</span>
									</li>
									<?
								}
		...
		if (CLASSIFIED_FEATURE == "on" && CUSTOM_CLASSIFIED_FEATURE == "on") {
			...
			if ($classifiedsRelatedSearch > 0) {
				...
				<ul class="list list-category">
					...
					<?
					if (CATEGORY_SCALABILITY_OPTIMIZATION != "on") {
						$categories = db_getFromDBBySQL("classifiedcategory", "SELECT * FROM ClassifiedCategory WHERE category_id = 0 AND enabled = 'y' ORDER BY $orderbyfield");
						if ($categories) {
							...
							foreach ($categories as $category) {
								...
								if ($thisclassifieds > 0) {
									?>
									<li class="level-2">
										<a href="<?=CLASSIFIED_DEFAULT_URL?>/results.php?keyword=<?=$keyword?>&amp;where=<?=$where?>&amp;category_id=<?=$this_category_id?>"><?=$category->getString("title")?></a> <span>(<?=$thisclassifieds?>)</span>
									</li>
									<?
								}
		...
		if (ARTICLE_FEATURE == "on" && CUSTOM_ARTICLE_FEATURE == "on") {
			...
			if ($articlesRelatedSearch > 0) {
				...
				<ul class="list list-category">
					...
					<?
					if (CATEGORY_SCALABILITY_OPTIMIZATION != "on") {
						$categories = db_getFromDBBySQL("articlecategory", "SELECT * FROM ArticleCategory WHERE category_id = 0 AND enabled = 'y' ORDER BY $orderbyfield");
						if ($categories) {
							...
							foreach ($categories as $category) {
								...
								if ($thisarticles > 0) {
									?>
									<li class="level-2">
										<a href="<?=ARTICLE_DEFAULT_URL?>/results.php?keyword=<?=$keyword?>&amp;where=<?=$where?>&amp;category_id=<?=$this_category_id?>"><?=$category->getString("title")?></a> <span>(<?=$thisarticles?>)</span>
									</li>
									<?
								}
                        ...
                        if ($categories) {
                            ...
                            foreach ($categories as $category) {
                                ...
                                if ($thispromotions > 0) {
                                    ...
                                    <li class="level-2">
                                        <a href="<?=PROMOTION_DEFAULT_URL?>/results.php?keyword=<?=$keyword?>&amp;where=<?=$where?>&amp;category_id=<?=$this_category_id?>"><?=$category->getString("title")?></a> <span>(<?=$thispromotions?>)</span>
                                    </li>
	...
	if (!$this_items) {
		...
		echo "<ul class=\"list list-category\">";
			...
			if (PROMOTION_FEATURE == "on" && CUSTOM_PROMOTION_FEATURE == "on" && CUSTOM_HAS_PROMOTION) {echo "<li class=\"level-1\"><a href=\"".PROMOTION_DEFAULT_URL."/\">".system_showText(LANG_MENU_PROMOTION)."</a></li>"; }
		echo "</ul>";
