<?
	...
	class DiscountCode extends Handle {
		...
		function makeFromRow($row="") {
			...
			$this->case					= ($row["case"])					? $row["case"]					: "";
			$this->location_1			= ($row["location_1"])				? $row["location_1"]					: "";
...
//			$dbMain->close();
			...
			if ($this->x_id) {
				...
				$sql = "INSERT INTO Discount_Code"
					. " (id, amount, type, status, expire_date, listing, event, banner, classified, article, `case`, location_1 ,recurring)"
					. " VALUES"
					. " ($this->id, $this->amount, $this->type, $this->status, $this->expire_date, $this->listing, $this->event, $this->banner, $this->classified, $this->article, $this->case, $this->location_1, $this->recurring)";
				$dbObj->query($sql);
