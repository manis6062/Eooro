<?
	 ...
	 * @method getArticleByFriendlyURL
	...
	class Article extends Handle {
		...
		function Delete($domain_id = false) {
			...
			### REVIEWS
			$sql = "SELECT id FROM Review WHERE item_type='article' AND item_id= $this->id AND status = 'A'";
			$result = $dbObj->query($sql);
       ...
       /**
        ...
        function getArticleToApp() {
            ...
            if ($this->id > 0 && $this->status == 'A') {
                 ...
                 */
                $add_info["friendly_url"] = ARTICLE_DEFAULT_URL."/".ALIAS_SHARE_URL_DIVISOR."/".$this->friendly_url;
