<?
	...
	class Location5 extends Handle {
		...
		function retrieveAllLocation($ids = false, $fields = "*") {
			$dbObj = db_getDBObject(DEFAULT_DB,true);
			$sql = "SELECT $fields FROM Location_5 ".(is_array($ids) ? "WHERE id IN (".implode(",", $ids).")" : "")." ORDER BY name";
			$result = $dbObj->query($sql);
            ...
            if ($allLocations) {
                ...
                */
                ...
                if (is_array($locations_id)) {
                    $sql = "SELECT * FROM Location_5 WHERE location_".$father_level." = ".$father_level_value." AND id IN (".  implode(",", $locations_id).") ORDER BY name";
                } else {
                    return false;
                }
            } else {
                $sql = "SELECT * FROM Location_5 WHERE location_".$father_level." = ".$father_level_value." ".(is_array($ids) ? " AND id IN (".implode(",", $ids).")" : "")." ORDER BY name";
            }
            $result = $dbObj->query($sql);
