<?
	...
	# ----------------------------------------------------------------------------------------------------
	# * FILE: /theme/diningguide/frontend/results_tabs.php
	# ----------------------------------------------------------------------------------------------------

    if ((GOOGLE_MAPS_ENABLED == "on" && $aux_module_items && $mapObj && $mapObj->getString("value") == "on" && $show_results) || ($show_tabs_on_no_results)) { 
        setting_get("gmaps_max_markers", $maxMarkers);
        $maxMarkers = ($maxMarkers ? $maxMarkers : GOOGLE_MAPS_MAX_MARKERS);
        ?>
        <ul class="tabs">
            <?
            if ($show_tabs_on_no_results) {
                ?>
