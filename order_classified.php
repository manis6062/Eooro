<?
	...
	if (!in_array($level, $classiLevelValue)) {
		header("Location: ".DEFAULT_URL."/".ALIAS_ADVERTISE_URL_DIVISOR.".php?classified");
		exit;
	}
	...
	if (($_SERVER['REQUEST_METHOD'] == "POST")) {
		...
		if ($validate_account && $validate_contact && $validate_classified && $validate_discount) {
			...
			if ($emailNotificationObj = system_checkEmail(SYSTEM_CLASSIFIED_SIGNUP)) {
				...
				system_mail($contact->getString("email"), $subject, $body, EDIRECTORY_TITLE." <$sitemgr_email>", $emailNotificationObj->getString("content_type"), "", $emailNotificationObj->getString("bcc"), $error, "", "", "", "", $contact->account_id, SYSTEM_CLASSIFIED_SIGNUP);
			}
			////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            $sitemgr_msg .= "<br /><a href=\"".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/view.php?id=".$classified->getNumber("id")."\" target=\"_blank\">".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/view.php?id=".$classified->getNumber("id")."</a><br /><br />";
